...
import goal
import oracles
from parse_input import read_flat_ints, parse_pms


# The coefficient for calculating next-round switch CPU usage of PMs. Next
# = (1-C)*accumulated + C*current
...
def calculate_set_weights(graph, assignment):
    max_pm_id_returned = max(assignment)
    set_weights = [0] * (max_pm_id_returned + 1)
    for i, pm in enumerate(assignment):
        set_weights[pm] += graph.node[i + 1][oracles.NODE_WEIGHT_KEY]
    return set_weights


def find_least_stressed_pm(num_pms, goal_values, switch_cpu_dist, vhost_cpu_dist):
    pm_cpu_usage = sorted([(switch_cpu_dist[i] + vhost_cpu_dist[i], i)
                           for i in range(num_pms) if goal_values[i] >= 1])
...
def move_vhosts_out_of_disabled_pms(graph, assignment, num_pms, goal_values, switch_cpu_dist, vhost_cpu_dist):
    ...
    for i, pm in enumerate(assignment):
        if len(goal_values) <= pm or goal_values[pm] < 1:
            ...
            new_pm = find_least_stressed_pm(
                num_pms, goal_values, switch_cpu_dist, vhost_cpu_dist)
            vhost_cpu_req = graph.node[i + 1][oracles.NODE_CPU_KEY]
            assignment[i] = new_pm
            vhost_cpu_dist[new_pm] += vhost_cpu_req
            print('  Moved vhost %d from disabled PM #%d to PM #%d.' %
                  (i, pm, new_pm))
            # Should have updated switch CPU usage of the PM as well to better
            # find the least stressed PM.


def main_loop_multi_constraint(oracle,
                               physical_machines,
                               vhost_cpu_values,
                               use_knapsack_limit=True,
                               stop_when_no_under_utilized_pms=False):
    num_pms = len(physical_machines)
    total_vhost_cpu_req = sum(vhost_cpu_values)
    # min_pms_needed = math.ceil(total_vhost_cpu_req / 100)
    switch_cpu_dist = [goal.INITIAL_CPU_PERCENT] * num_pms
    # TODO: must think about the relation between available CPU shares and total CPU shares needed.
    vhost_cpu_dist = [goal.MAX_CPU_PERCENT - v for v in switch_cpu_dist]
    capacity_goal_calc = goal.CapacityGoalCalculator(
        oracle.graph, physical_machines, use_knapsack_limit)
    capacity_goal_values = capacity_goal_calc.get_next_goal(switch_cpu_dist, vhost_cpu_dist)
    load_imbalance_vec = [1.0, goal.OVER_UTILIZED_CPU_THRESHOLD / goal.MAX_CPU_PERCENT]
    assignment = oracle.get_assignment(capacity_goal_values, vhost_cpu_dist, load_imbalance_vec)
    print(assignment)
    new_vhost_cpu_dist = calculate_vhost_cpu_dist(oracle.graph, assignment, num_pms)
    print(new_vhost_cpu_dist)
    print(switch_cpu_dist)
    print(calculate_set_weights(oracle.graph, assignment))
...
def main_loop(oracle, physical_machines,
    ...
    if correct_goals_by_scaling:
        correction_approach = goal.CapacityGoalCalculator.CORRECTION_APPROACH_SCALE
    else:
        correction_approach = goal.CapacityGoalCalculator.CORRECTION_APPROACH_REDUCE
    goal_calc = goal.CapacityGoalCalculator(
        oracle.graph, physical_machines, use_knapsack_limit, correction_approach=correction_approach)
    ...
    while True:
        ...
        new_vhost_cpu_dist = calculate_vhost_cpu_dist(
            oracle.graph, assignment, num_pms)
        move_vhosts_out_of_disabled_pms(
            oracle.graph, assignment, num_pms, goal_values, switch_cpu_dist, new_vhost_cpu_dist)
        print('\nAssignment result:\n' + str(assignment))
        ...
        for i in range(num_pms):
            ...
            new_switch_cpu_val = max(
                goal.MIN_CPU_PERCENT, goal.MAX_CPU_PERCENT - new_vhost_cpu_val)
            total_cpu_usage = old_switch_cpu_val + new_vhost_cpu_val
            if total_cpu_usage < goal.UNDER_UTILIZED_CPU_THRESHOLD:
                status = 'under'
                under_utilized_pms.append(
                    (physical_machines[i], new_vhost_cpu_val))
                ...
                status = 'ok'
            switch_cpu_dist[i] = math.ceil(
                (1 - PM_SW_CPU_UPDATE_RATIO) * switch_cpu_dist[i] + PM_SW_CPU_UPDATE_RATIO * new_switch_cpu_val)
            ...
            #         vhost_cpu_dist[i] = pm_cpu_ceiling[i] - switch_cpu_dist[i]
            print(' PM #%d: %s, CPU=%d/%d/%d, next_sw_CPU=(%.2lf*%d)+(%.2lf*%d)=%d, next_vhost_CPU = %d.' % (i, status, new_vhost_cpu_val, old_switch_cpu_val+new_vhost_cpu_dist[
                  i], pm_cpu_ceiling[i], (1 - PM_SW_CPU_UPDATE_RATIO), old_switch_cpu_val, PM_SW_CPU_UPDATE_RATIO, new_switch_cpu_val, switch_cpu_dist[i], vhost_cpu_dist[i]))
        num_over_utilized_pms = len(over_utilized_pms)
        num_under_utilized_pms = len(under_utilized_pms)
        # For adaptive reduction, we reduce *the most stressed* PM when *same
        # thing happens again*.
        if num_under_utilized_pms <= goal.UNDER_UTILIZED_PM_ALLOWED and num_over_utilized_pms == 0:
            ...
            print(
                'All PMs are over-utilized. Stop because we probably cannot do better.')
            break
        elif stop_when_no_under_utilized_pms and num_under_utilized_pms == 0:
            print(
                'PMs are either ok or over. There might be space for balancing load but...')
            break
        elif assignment in assignment_history:
            print('Assignment result repeats round %d.' %
                  (assignment_history.index(assignment)))
...
def main():
    parser = argparse.ArgumentParser(
        description='Wrapper to graph partitioning oracle.')
    parser.add_argument(
        '--oracle', type=str, default='chaco', help='Graph partitioning oracle.')
    parser.add_argument(
        '--graph-file', type=str, required=True, help='File to read input graph from.')
    ...
    parser.add_argument('--disable-knapsack-limit', default=False, action='store_true',
                        help='If set, will not use knapsack limit when computing capacity.')
    parser.add_argument('--disable-adaptive-pm-cpu-ceiling', default=False, action='store_true',
                        help='If set, will not adaptively reduce CPU ceiling value for PMs.')
    parser.add_argument('--stop-if-no-under-pms', default=False, action='store_true',
                        help='If set, will stop when there is no PM that is under-utilized.')
    parser.add_argument('--correct-goals-by-scaling', default=False, action='store_true',
                        help='If set, will scale the goals to vertex weight sum rather than subtracting.')
    args = parser.parse_args()

    # Update the graph with vhost CPU requirement.
    vhost_cpu_values = read_flat_ints(args.vhost_cpu_file)
    ...
    # Read PM information.
    physical_machines = parse_pms(args.pm_file)

    if args.oracle.lower() == 'chaco':
        oracle.update_vhost_cpu_req(vhost_cpu_values)
    ...
    if args.oracle.lower() == 'metis':
        oracle.update_vhost_cpu_req(vhost_cpu_values, True)
        main_loop_multi_constraint(oracle=oracle,
                  physical_machines=physical_machines,
                  vhost_cpu_values=vhost_cpu_values,
                  use_knapsack_limit=not args.disable_knapsack_limit,
                  stop_when_no_under_utilized_pms=args.stop_if_no_under_pms)
