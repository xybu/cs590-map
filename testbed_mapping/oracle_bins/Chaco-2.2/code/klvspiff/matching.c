...
void bpcover(n_left, n_right, pointers, indices, sep_size,
             sep_nodes) int n_left; /* number of vertices on left side */
...
int *sep_nodes;                     /* list of separator nodes */
{
  ...
  *sep_size = 0;
  ...
  if (DEBUG_COVER) {
    confirm_match(n_left, n_right, pointers, indices, matching, *sep_size,
                  sep_nodes);
  }
...
static void bpmatching(
    n_left, n_right, pointers, indices, matching,
...
static void augment(node, pointers, indices, matching, touched,
...
static int touch(node, pointers, indices, matching, touched, seen,
                 nseen) int node;
int *pointers; /* start/stop of adjacency lists */
int *indices;  /* adjacency list for each vertex */
int *matching; /* array to encode matching */
...
static int touch2(node, pointers, indices, matching, touched) int node;
int *pointers; /* start/stop of adjacency lists */
int *indices;  /* adjacency list for each vertex */
int *matching; /* array to encode matching */
int *touched;  /* flags for each vertex */
  ...
  for (j = pointers[node]; j < pointers[node + 1]; j++) {
    ...
    if (!touched[neighbor]) {
      ...
      if (matching[neighbor] == -1) {
        ...
        result =
            touch2(matching[neighbor], pointers, indices, matching, touched);
        if (result) {
          return (TRUE);
        }
      }
...
void confirm_match(n_left, n_right, pointers, indices, matching, sep_size,
                   sep_nodes) int n_left; /* number of vertices on left side */
int n_right;                              /* number of vertices on right side */
int *pointers;                            /* start/stop of adjacency lists */
int *indices;                             /* adjacency list for each vertex */
  ...
  for (i = 0; i < n_left + n_right; i++) {
    if (matching[i] != -1 && matching[matching[i]] != i) {
      printf("ERROR: matching[%d] = %d, but matching[%d] = %d\n", i,
             matching[i], matching[i], matching[matching[i]]);
    }
  }
}

int match_size(matching, nleft) int *matching, nleft;
{
  int i, nmatch;
