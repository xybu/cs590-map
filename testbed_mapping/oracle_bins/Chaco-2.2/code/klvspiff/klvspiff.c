...
void klvspiff(
    graph, nvtxs, sets, goal, max_dev, bndy_list,
    weights) struct vtx_data **graph; /* list of graph info for each vertex */
...
double *weights; /* vertex weights in each set */
{
  ...
  time1 = seconds();
  error = klv_init(&lbuckets, &rbuckets, &llistspace, &rlistspace, &ldvals,
                   &rdvals, nvtxs, maxdval);
  ...
  if (!error) {
    ...
    time1 = seconds();
    error = nway_klv(graph, nvtxs, lbuckets, rbuckets, llistspace, rlistspace,
