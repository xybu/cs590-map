...
void balance(graph, nvtxs, nedges, using_vwgts, using_ewgts, vwsqrt, igeom,
             coords, assignment, goal, architecture, ndims_tot, mesh_dims,
             global_method, local_method, rqi_flag, vmax, ndims, eigtol,
             hops) struct vtx_data **graph; /* data structure for graph */
...
short (*hops)[MAXSETS]; /* between-set hop cost for KL */
{
  ...
  set_buckets = (struct set_info **)smalloc((unsigned)(nsets_tot + 1) *
                                            sizeof(struct set_info));
  set_info =
      (struct set_info *)smalloc((unsigned)nsets_tot * sizeof(struct set_info));
  ...
  while (max_proc_size > 1) { /* Some set still needs to be divided. */
    ...
    hops_flag =
        divide_procs(architecture, ndims, ndims_tot, set_info, set, subsets,
                     (global_method == 3), &ndims_real, &nsets_real, &striping,
                     cut_dirs, mesh_dims, hops_special);

    /* If new cut direction, turn off terminal propagation. */
    new_dir = FALSE;
    if (architecture == 0) {
      if (old_max_proc_size != max_proc_size) new_dir = TRUE;
    } else if (architecture > 0) {
      if (!done_dir[cut_dirs[0]]) new_dir = TRUE;
    ...
    if (pass == 1) { /* First partition. */
      ...
      merge_goals(goal, merged_goal, set_info, subsets, nsets_real, ndims_tot,
                  architecture, mesh_dims, sub_vwgt_sum);
    }
      ...
      if (pass == 2) {
        ...
        if (!using_vwgts) {
          ...
          subvwsqrt =
              (double *)smalloc((unsigned)(maxsize + 1) * sizeof(double));
        }

        if (graph != NULL) {
          subgraph = (struct vtx_data **)smalloc((unsigned)(maxsize + 1) *
        ...
        if (global_method == 3 ||
          ...
          subcoords[0] =
              (float *)smalloc((unsigned)(maxsize + 1) * sizeof(float));
          if (igeom > 1)
            subcoords[1] =
                (float *)smalloc((unsigned)(maxsize + 1) * sizeof(float));
          if (igeom > 2)
        ...
        if (TERM_PROP && graph != NULL) {
          term_wgts = (float *)smalloc((unsigned)(nsets - 1) * (maxsize + 1) *
                                       sizeof(float));
      ...
      subnvtxs =
          make_maps(setlists, list_ptrs, set->setnum, glob2loc, loc2glob);

      if (TERM_PROP && !new_dir && graph != NULL) {
        all_term_wgts[1] = save_term_wgts;
        make_term_props(graph, subnvtxs, loc2glob, assignment, architecture,
                        ndims_tot, ndims_real, set_info, set->setnum,
                        nsets_real, nsets_tot, subsets, all_term_wgts,
                        using_ewgts);
      } else
        all_term_wgts[1] = NULL;
    ...
    if (DEBUG_TRACE > 1) {
      printf("About to call divide with nvtxs = %d, nedges = %d, ", subnvtxs,
             subnedges);
      if (!architecture)
        printf("ndims = %d\n", set->ndims);
    ...
    divide(subgraph, subnvtxs, subnedges, using_vwgts, using_ewgts, subvwsqrt,
           igeom, subcoords, subassign, merged_goal, architecture,
           all_term_wgts, global_method, local_method, rqi_flag, vmax,
           ndims_real, eigtol, (hops_flag ? hops_special : hops), nsets_real,
           striping);
    ...
    if (pass == 1) {
      ...
      if (max_proc_size > 1) {
        ...
        maxsize = 0;
        for (i = 0; i < nsets; i++) {
          if (setsize[i] > maxsize) maxsize = setsize[i];
        }
      }
