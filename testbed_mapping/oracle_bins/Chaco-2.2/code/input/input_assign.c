...
int input_assign(finassign, inassignname, nvtxs,
                 assignment) FILE *finassign; /* input assignment file */
...
short *assignment;   /* values to be printed */
{
  ...
  flag = 0;
  if (assignment[0] > nvtxs) flag = assignment[1];
  for (i = 1; i < nvtxs; i++) {
    ...
    if (assignment[i] > nvtxs) { /* warn since probably an error */
      if (assignment[i] > flag) flag = assignment[i];
    }
  }
