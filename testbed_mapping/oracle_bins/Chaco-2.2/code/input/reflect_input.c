...
void reflect_input(nvtxs, nedges, igeom, graphname, geomname, inassignname,
                   outassignname, outfilename, architecture, ndims_tot,
                   mesh_dims, global_method, local_method, rqi_flag, vmax,
                   ndims, eigtol, seed,
                   outfile) int nvtxs; /* number of vertices in graph */
...
FILE *outfile;       /* file to write output to */
{
  ...
  for (i = 0; i < 2; i++) {
    ...
    if (architecture == 0) {
      ...
      fprintf(tempfile, "Partitioning target: %d-dimensional mesh of size ",
              architecture);
      if (architecture == 1)
        fprintf(tempfile, "%d\n", mesh_dims[0]);
...
static void reflect_params(tempfile, global_method, local_method, rqi_flag,
                           ndims) FILE *tempfile; /* file or stdout */
int global_method; /* global partitioning algorithm */
int local_method;  /* local partitioning algorithm */
int rqi_flag;      /* use RQI/SYMMLQ eigensolver? */
int ndims;         /* number of eigenvectors to generate */
{
  ...
  if (global_method == 1) {
    ...
    /*
            if (LIMIT_KL_EWGTS) {
                fprintf(tempfile, "  LIMIT_KL_EWGTS = %s, EWGT_RATIO_MAX =
       %g\n",
  ...
  if (DEBUG_PERTURB) fprintf(tempfile, "  DEBUG_PERTURB = %d\n", DEBUG_PERTURB);
  if (DEBUG_ASSIGN) fprintf(tempfile, "  DEBUG_ASSIGN = %d\n", DEBUG_ASSIGN);
  if (DEBUG_INERTIAL)
    fprintf(tempfile, "  DEBUG_INERTIAL = %d\n", DEBUG_INERTIAL);
  if (DEBUG_OPTIMIZE)
    fprintf(tempfile, "  DEBUG_OPTIMIZE = %d\n", DEBUG_OPTIMIZE);
  if (DEBUG_BPMATCH) fprintf(tempfile, "  DEBUG_BPMATCH = %d\n", DEBUG_BPMATCH);
  if (DEBUG_COARSEN) fprintf(tempfile, "  DEBUG_COARSEN = %d\n", DEBUG_COARSEN);
  if (DEBUG_EVECS) fprintf(tempfile, "  DEBUG_EVECS = %d\n", DEBUG_EVECS);
  if (DEBUG_KL) fprintf(tempfile, "  DEBUG_KL = %d\n", DEBUG_KL);
  if (DEBUG_MEMORY) fprintf(tempfile, "  DEBUG_MEMORY = %d\n", DEBUG_MEMORY);
  if (DEBUG_INPUT) fprintf(tempfile, "  DEBUG_INPUT = %d\n", DEBUG_INPUT);
  if (DEBUG_PARAMS) fprintf(tempfile, "  DEBUG_PARAMS = %d\n", DEBUG_PARAMS);
  if (DEBUG_INTERNAL)
  ...
  if (DEBUG_TRACE) fprintf(tempfile, "  DEBUG_TRACE = %d\n", DEBUG_TRACE);
