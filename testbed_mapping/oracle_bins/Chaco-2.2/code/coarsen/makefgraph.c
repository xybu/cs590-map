...
void makefgraph(
    graph, nvtxs, nedges, pcgraph, cnvtxs, pcnedges, v2cv, using_ewgts, igeom,
    coords, ccoords) struct vtx_data **graph; /* array of vtx data for graph */
...
float **ccoords; /* coordinates for coarsened vertices */
{
  ...
  *pcgraph = cgraph = (struct vtx_data **)smalloc((unsigned)(cnvtxs + 1) *
                                                  sizeof(struct vtx_data *));
  links =
      (struct vtx_data *)smalloc((unsigned)cnvtxs * sizeof(struct vtx_data));
  ...
  for (cvtx = 1; cvtx <= cnvtxs; cvtx++) {
    ...
    for (i = cv2v_ptrs[cvtx + 1] - cv2v_ptrs[cvtx]; i; i--) {
      ...
      if (using_ewgts) fptr = graph[vtx]->ewgts;
      for (j = graph[vtx]->nedges - 1; j; j--) {
        ...
        if (cneighbor != cvtx) {
          ...
          if (seenflag[cneighbor] == 0) { /* New neighbor. */
            ...
            if (COARSEN_EWGTS) cgptr->ewgts[nseen] = ewgt;
            seenflag[cneighbor] = nseen++;
          } else { /* Already seen neighbor. */
            if (COARSEN_EWGTS) cgptr->ewgts[seenflag[cneighbor]] += ewgt;
          }
        } else if (using_ewgts)
          ++fptr;
      }
    ...
    if (COARSEN_EWGTS) cgptr->ewgts[0] = -ewgt_sum;
  ...
  if (COARSEN_EWGTS) {
    ewptr = eweights;
    eweights =
        (float *)srealloc((char *)eweights, (unsigned)size * sizeof(float));
...
static void makecv2v(nvtxs, cnvtxs, v2cv, cv2v_vals,
                     cv2v_ptrs) int nvtxs; /* number of vertices in graph */
int cnvtxs;     /* number of vtxs in coarsened graph */
int *v2cv;      /* mapping from vtxs to coarsened vtxs */
int *cv2v_vals; /* vtxs corresponding to each cvtx */
