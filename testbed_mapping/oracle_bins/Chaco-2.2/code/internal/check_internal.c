...
void check_internal(
    graph, nvtxs, int_list, set_list, vtx_elems, total_vwgt, assign,
    nsets_tot) struct vtx_data **graph; /* graph data structure */
...
int nsets_tot;            /* total number of sets */
{
  ...
  for (ptr = int_list[nsets_tot].next; ptr != NULL; ptr = ptr->next) {
    ...
    if (ptr->prev != old_ptr) {
      printf(
          " int_list back link screwed up, set=%d, k=%d, old_ptr=%ld, "
          "ptr->prev = %ld\n",
    ...
    for (ptr2 = set_list[set].next; ptr2 != NULL; ptr2 = ptr2->next) {
      ...
      if (ptr2->prev != old_ptr2) {
        printf(
            " set_list back link screwed up, set=%d, k=%d, old_ptr2=%ld, "
            "ptr2->prev = %ld\n",
