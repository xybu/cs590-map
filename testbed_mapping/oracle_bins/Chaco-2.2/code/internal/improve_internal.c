...
int improve_internal(
    graph, nvtxs, assign, goal, int_list, set_list, vtx_elems, set1, locked,
    nlocked, using_ewgts, vwgt_max,
    total_vwgt) struct vtx_data **graph; /* graph data structure */
...
int *total_vwgt;          /* total vertex weight in each set */
{
  ...
  for (i = 1; i < graph[best_vtx]->nedges && flag; i++) {
    ...
    if (set2 != set1) {
      ...
      balanced = (total_vwgt[set1] - goal[set1] * ratio + goal[set2] * ratio -
                  total_vwgt[set2]) <= vwgt_max;
      while (!balanced && flag) {
        ...
        balanced =
            total_vwgt[set1] - goal[set1] + goal[set2] - total_vwgt[set2] <=
            vwgt_max;
      }
    }
  }
