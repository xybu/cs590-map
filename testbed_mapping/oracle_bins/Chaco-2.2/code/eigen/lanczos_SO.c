...
/* These comments are for version 1 of selective orthogonalization:
   ...
   orthogonality. Specifically, the pauses to check for convergence of Ritz
   pairs
   at first come at a small but increasing interval as the computation builds
   up.
   Later they come at a regular, pre-set interval, e.g. every 10 steps. Hence
   this
   is similar to Grear's periodic reorthogonalization scheme, but with an
   adaptive
   period. A small number of ritz pairs at both ends of the spectrum are
   monitored
   for convergence at each pause. The number monitored gradually increases as
   more
   Ritz pairs converge so that the number monitored at each pause is always at
   least
   ...
   the wrong eigenpair. These conditions will usually be detected by the
   algorithm
   and a warning issued. If the graph is small, full orthogonalization is
   then a fall back option. If the graph is large, one of the multi-level
   schemes
   ...
   This is more expensive then simply taking it out of the residual on each
   step,
   ...
   on the Sturm sequence, whichever is faster based on a simple complexity
   model.
...
/* These comments are for version 2 of selective orthogonalization:
   ...
   make it faster. Now only checks left end of spectrum for converging Ritz
   pairs.
   Only checking the left end of the spectrum for converging Ritz pairs is in
   principle
   not as good as checking both ends, but in practice seems to be no worse and
   usually
   better than checking both ends. Since the distribution of eigenvalues for
   Laplacian
   graphs of interest seems to generally result in much more rapid convergence
   on the
   right end of the spectrum, ignoring the converging Ritz pairs there saves a
   lot of
   work. */
...
/* These comments explain some of indexing conventions used:
   Note that the indexing of the tridiagonal matrix is consistent with Parlett
   and
   Scott's Math Comp  '79 paper "The Lanczos Algorithm With Selective
   Orthogonalization"
   and with Golub and Van Loan's (2nd ed.) treatment in Chapter 9. So on
   iteration j
   the tridiagonal matrix T has diagonal = alpha[1], alpha[2] ... alpha[j] and
   off-diagonal = beta[1], beta[2] ... beta[j-1]. beta[0] is the norm of the
   initial
...
/* These comments pertain to the calculation of the eigenvalues of T:
   ...
   Computation is first attempted using either the QL algorithm or Sturm
   sequence
   bisection, whichever is predicted to be cheaper based on simple complexity
   analysis
   (normally bisection is cheaper in this context). The classic bisection
   algorithm
   can fail due to overflow, so a re-scaling heuristic is used to guard against
   this.
   The number of bisection and QL iterations is monitored and if a reasonable
   maximum
   is observed these routines abort. If either routine fails, the other is
   applied
   to the problem. If both routines fail, Lanczos is backed up to the last pause
   point
   and an approximate eigenvector is computed based on the available
   information. */
...
void lanczos_SO(
    A, n, d, y, lambda, bound, eigtol, vwsqrt, maxdeg, version, cube_or_mesh,
    nsets, assignment, active, mediantype, goal,
    vwgt_max) struct vtx_data **A; /* sparse matrix in row linked list format */
int n;                             /* problem size */
...
int vwgt_max;      /* largest vertex weight */
{
  ...
  struct scanlink
      *scanlist;           /* linked list of fields to do with min ritz vals */
  ...
  solist = (struct orthlink **)smalloc((unsigned)(maxj + 1) *
                                       sizeof(struct orthlink *));
  ...
  interval =
      2 + (int)min(LANCZOS_SO_INTERVAL - 2, n / (2 * LANCZOS_SO_INTERVAL));
  ...
  if (DEBUG_EVECS > 0) {
    ...
    printf("  maxj %d\n", maxj);
    printf("  srestol %g\n", SRESTOL);
    if (LANCZOS_CONVERGENCE_MODE == 1) printf("  assigntol %d\n", assigntol);
  }
  ...
  while ((j <= maxj) && (!converged) && (ritzval_flag == 0) && memory_ok) {
    ...
    if (q[j] == NULL) {
      ...
      if (DEBUG_EVECS > 0 || WARNING_EVECS > 0) {
        strout(
            "WARNING: Lanczos out of memory; computing best approximation "
            "available.\n");
      }
      if (nopauses) {
    ...
    time = lanc_seconds();
    pause =
        lanpause(j, lastpause, interval, q, n, &pausemode, version, beta[j]);
    ...
    if (pause) {
      ...
      ritzval_flag =
          get_ritzvals(alpha, beta, j, Anorm, workj, ritz, d, left_goodlim,
                       right_goodlim, eigtol, bis_safety);
      ql_time += lanc_seconds() - time;

      /* If get_ritzvals() fails, back up to last pause point and exit main
       * loop. */
      if (ritzval_flag != 0) {
        if (DEBUG_EVECS > 0 || WARNING_EVECS > 0) {
          strout(
              "ERROR: Lanczos failed in computing eigenvalues of T; computing");
          strout(
              "       best readily available approximation to eigenvector.\n");
        }
        if (firstpause) {
          bail("ERROR: Sorry, can't salvage Lanczos.", 1);
          /* ... save yourselves, men.  */
      ...
      if (LANCZOS_CONVERGENCE_MODE == 1 && d == 1) {
        ...
        if (firstpause) {
          ...
          if (!converged) {
            ...
            mkeigvecs(scanlist, lambda, bound, index, bj, d, &Sres_max, alpha,
                      beta, j, s, y, n, q);

            /* compute first assignment */
            assign(A, y, n, d, cube_or_mesh, nsets, vwsqrt, assignment, active,
          ...
          old_assgn_pntr = old_assignment;
          for (i = n + 1; i; i--) {
            if (*old_assgn_pntr++ != *assgn_pntr++) assigndiff++;
          }
      ...
      if (DEBUG_EVECS > 2 && !converged) {
        ...
        /* compute current approx. to eigenvectors */
        mkeigvecs(scanlist, lambda, bound, index, bj, d, &Sres_max, alpha, beta,
                  j, s, y, n, q);

        /* Compute residuals and display associated info. */
        printf(
            "j %4d;    lambda                Ares est.             Ares        "
            "  index\n",
            j);
        for (i = 1; i <= d; i++) {
      ...
      if (!converged) {
        ...
        if (DEBUG_EVECS > 2) {
          time = lanc_seconds();
          printf("  j %3d; goodlim lft %2d, rgt %2d; list ", j, left_goodlim,
                 right_goodlim);
  ...
  time = lanc_seconds();
  mkeigvecs(scanlist, lambda, bound, index, bj, d, &Sres_max, alpha, beta, j, s,
            y, n, q);
  evec_time += lanc_seconds() - time;

  /* Analyze computation for and report additional problems */
  time = lanc_seconds();
  warnings(workn, A, y, n, lambda, vwsqrt, Ares, bound, index, d, j, maxj,
           Sres_max, eigtol, u, Anorm, Output_File);
  debug_time += lanc_seconds() - time;
