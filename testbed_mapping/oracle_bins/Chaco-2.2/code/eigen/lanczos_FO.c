...
void lanczos_FO(A, n, d, y, lambda, bound, eigtol, vwsqrt, maxdeg,
                version) struct vtx_data **A; /* graph data structure */
...

{
  ...
  if (DEBUG_EVECS > 0) {
    if (version == 1) {
      ...
      printf(
          "Full orthogonalization Lanczos, inverted operator, matrix size = "
          "%d\n",
          n);
    }
  }
  ...
  if (version == 2) {
    ...
    normxlim =
        DOUBLE_MAX; /* Effectively disables ||x|| termination criterion */
  ...
  while ((j <= maxj) && (converged == FALSE) && memory_ok) {
    ...
    /* Allocate next Lanczos vector. If fail, back up one step and compute
     * approx. eigvec. */
    q[j] = mkvec_ret(1, n);
    if (q[j] == NULL) {
      memory_ok = FALSE;
    ...
    if (version == 1) {
      ...
      symmlq_(&long_n, &(q[j][1]), &wv1[1], &wv2[1], &wv3[1], &wv4[1], &Aq[1],
              &wv5[1], &wv6[1], &checka, &goodb, &precon, &shift, &nout,
              &intlim, &rtol, &istop, &itn, &anorm, &acond, &rnorm, &ynorm,
              (double *)A, vwsqrt, (double *)orthlist2, &macheps, &normxlim,
              &itnmin);
      symmlqitns += itn;
      if (DEBUG_EVECS > 2) {
    ...
    ritzval_flag = get_ritzvals(alpha, beta + 1, j, Anorm, workj + 1, ritz, d,
                                left, right, eigtol, bis_safety);
  ...
  time = lanc_seconds();
  mkeigvecs(scanlist, lambda, bound, index, bj, d, &Sres_max, alpha, beta + 1,
            j, s, y, n, q);
