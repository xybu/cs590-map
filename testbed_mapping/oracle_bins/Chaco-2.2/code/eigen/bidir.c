...
double bidir(alpha, beta, j, ritz, s,
             hurdle) double *alpha; /* vector of Lanczos scalars */
...

{
  ...
  while (i <= j && iterate) {
    ...
    s[i] =
        -((alpha[i - 1] - ritz) * s[i - 1] + beta[i - 1] * s[i - 2]) / beta[i];
  ...
  if (!iterate) {
    ...
    for (i = j; i >= k + 2; i--) {
      s[i - 2] =
          -((alpha[i - 1] - ritz) * s[i - 1] + beta[i] * s[i]) / beta[i - 1];
    }
    scale = s[k] / sav_val;
    for (i = 1; i <= k - 1; i++) {
      s[i] = s[i] * scale;
    ...
    residual =
        beta[k] * s[k - 1] + (alpha[k] - ritz) * s[k] + beta[k + 1] * s[k + 1];
  } else {
