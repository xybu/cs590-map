...
/* This is based on lanczos_SO but is simplified and tailored to solve
   the extended eigen-problem using Rafael's technique. Solves Nx = lambda*Wx +
   Wg.
   In the terminal propagation context we know that e^T Wg = 0 by virue of the
   way
   g is constructed. This code, however, is designed to work in the more general
   case of arbitrary g (but this capability has not been tested). Returns 0 if
   it
   thinks everything's OK, nonzero otherwise. */

int lanczos_ext(
    A, n, d, y, eigtol, vwsqrt, maxdeg, version, gvec,
    sigma) struct vtx_data **A; /* sparse matrix in row linked list format */
int n;                          /* problem size */
                   ...
                   eigenvector */
{
  ...
  if (DEBUG_EVECS > 0) {
    printf(
        "Selective orthogonalization Lanczos for extended eigenproblem, matrix "
        "size = %d.\n",
        n);
  }
  ...
  if (check > 10 * numerical_zero && WARNING_EVECS > 0) {
    strout(
        "WARNING: In terminal propagation, rhs should have no component in "
        "the");
    printf(
        "         nullspace of the Laplacian, so check val %g should be "
        "negligible.\n",
        check);
    if (Output_File != NULL) {
      fprintf(Output_File,
  ...
  while ((j <= maxj) && (!converged) && memory_ok) {
    ...
    if (q[j] == NULL) {
      ...
      if (DEBUG_EVECS > 0 || WARNING_EVECS > 0) {
        strout(
            "WARNING: Lanczos_ext out of memory; computing best approximation "
            "available.\n");
      }
      if (nopauses) {
    ...
    if (pause) {
      ...
      get_extval(alpha, beta, j, ritz[1], s, eigtol, beta[0], sigma, &extval, v,
                 work1, work2);
      ...
      if (!converged) {
        ...
        if (DEBUG_EVECS > 2) {
          ...
          checkeig_ext(workn, u, A, y[1], n, extval, vwsqrt, gvec, eigtol,
                       FALSE);
