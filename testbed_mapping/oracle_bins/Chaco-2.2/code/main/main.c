...
// In interface.c.
extern int calculate_nsets_total(int, int, int *);
...
int main() {
  ...
  char goalname[NAME_LENGTH];     /* name of goal input file */
  ...
  while (another) {
    ...
    use_goal_file = goalname[0] != '\0' && goalname[0] != '\n';
    if (use_goal_file) {
      int i;
      ...
      if (!gf) {
        perror("fopen");
        printf("ERROR: Failed to open goal file \"%s\". Use default instead.\n", goalname);
      } else if (!(goal = (double *)malloc((unsigned)nsets_tot * sizeof(double)))) {
        printf("ERROR: Failed to allocate memory for goal array! Use default.\n");
        fclose(gf);
      } else {
        for (i = 0; i < nsets_tot; ++i) {
          if (fscanf(gf, "%lf", goal + i) != 1) {
            perror("fscanf");
            printf("ERROR: Failed to read the %d-th goal. Use default.", i);
            free(goal);
            goal = NULL;
            fclose(gf);
            break;
    ...
    interface(nvtxs, start, adjacency, vwgts, ewgts, x, y, z, outassignptr,
              outfileptr, assignment, architecture, ndims_tot, mesh_dims, goal,
