...
int calculate_nsets_total(int architecture, int ndims_tot, int *mesh_dims) {
  if (architecture == 0)
    return 1 << ndims_tot;
  else if (architecture == 1)
    return mesh_dims[0];
  else if (architecture == 2)
    return mesh_dims[0] * mesh_dims[1];
  else /* if (architecture > 2) */
    return mesh_dims[0] * mesh_dims[1] * mesh_dims[2];
}
...
long seed;                     /* for random graph mutations */
{
  ...
  if (goal ==
    ...
    default_goal = TRUE;
    nsets_tot = calculate_nsets_total(architecture, ndims_tot, mesh_dims);
    ...
    for (i = 0; i < nsets_tot; i++) goal[i] = vwgt_sum;
  } else {
    default_goal = FALSE;
  }
