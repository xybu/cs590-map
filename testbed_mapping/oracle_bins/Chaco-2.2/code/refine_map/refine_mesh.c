...
int refine_mesh(comm_graph, cube_or_mesh, mesh_dims, maxdesire, vtx2node,
                node2vtx) struct vtx_data *
    *comm_graph;  /* graph for communication requirements */
...
short *node2vtx;  /* mapping from mesh nodes to comm_graph vtxs */
{
  ...
  vdata = (struct refine_vdata *)smalloc_ret(
      (unsigned)(cube_or_mesh * nsets_tot + 1) * sizeof(struct refine_vdata));
  ...
  edata = (struct refine_edata *)smalloc_ret((unsigned)(nwires + 1) *
                                             sizeof(struct refine_edata));
  ...
  for (wire = 0; wire < nwires; wire++) {
    desires[wire] = edata[wire].swap_desire = compute_mesh_edata(
        &(edata[wire]), vdata, mesh_dims, comm_graph, node2vtx);
  }

  /* Set special value for end pointer. */
  edata[nwires].swap_desire =
  ...
  if (best_desire > 0) {
    desire_ptr = (struct refine_edata **)smalloc_ret(
        (unsigned)(2 * imax + 1) * sizeof(struct refine_edata *));
  ...
  /* Everything is now set up.  Swap sets across wires until no more
   * improvement. */
  ...
  while (best_desire > 0) {
    ...
    for (j = 1; j < comm_graph[vtx1]->nedges; j++) {
      ...
      if (neighbor != vtx2)
        update_mesh_vdata(loc1, loc2, dim, comm_graph[vtx1]->ewgts[j], vdata,
                          mesh_dims, neighbor, vtx2node);
    }

    for (j = 1; j < comm_graph[vtx2]->nedges; j++) {
      neighbor = comm_graph[vtx2]->edges[j];
      if (neighbor != vtx1)
        update_mesh_vdata(loc2, loc1, dim, comm_graph[vtx2]->ewgts[j], vdata,
                          mesh_dims, neighbor, vtx2node);
    }

    /* Now recompute all preferences for vertices that were moved. */
    for (j = 0; j < cube_or_mesh; j++) {
      compute_mesh_vdata(&(vdata[j * nsets_tot + vtx1]), comm_graph, vtx1,
                         vtx2node, mesh_dims, j);
      compute_mesh_vdata(&(vdata[j * nsets_tot + vtx2]), comm_graph, vtx2,
                         vtx2node, mesh_dims, j);
    }
    ...
    for (j = 1; j < comm_graph[vtx1]->nedges; j++) {
      neighbor = comm_graph[vtx1]->edges[j];
      if (neighbor != vtx2)
        update_mesh_edata(neighbor, dim, edata, vdata, comm_graph, mesh_dims,
    ...
    for (j = 0; j < cube_or_mesh; j++) {
      update_mesh_edata(vtx1, j, edata, vdata, comm_graph, mesh_dims, node2vtx,
                        vtx2node, &best_desire, imax, desire_ptr);
      update_mesh_edata(vtx2, j, edata, vdata, comm_graph, mesh_dims, node2vtx,
                        vtx2node, &best_desire, imax, desire_ptr);
    }
