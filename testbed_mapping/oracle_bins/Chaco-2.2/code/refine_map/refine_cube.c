...
int refine_cube(comm_graph, ndims_tot, maxdesire, vtx2node,
...
short *node2vtx;  /* mapping from processors to comm_graph vtxs */
{
  ...
  vdata = (struct refine_vdata *)smalloc_ret(
      (unsigned)(ndims_tot * nsets_tot + 1) * sizeof(struct refine_vdata));
  ...
  edata = (struct refine_edata *)smalloc_ret((unsigned)(nwires_tot + 1) *
  ...
  while (best_desire > 0) {
    ...
    for (j = 0; j < ndims_tot; j++) {
      k = 1 << j;
      compute_cube_vdata(&(vdata[j * nsets_tot + vtx1]), comm_graph, vtx1, k,
                         vtx2node);
      compute_cube_vdata(&(vdata[j * nsets_tot + vtx2]), comm_graph, vtx2, k,
                         vtx2node);
    }
    ...
    /* For each neighbor vtx, look at wire in this direction.  If desire hasn't
       changed,
       return.  Otherwise, pick him up and move him in desire list. Similarly
       for all
    ...
    for (j = 0; j < ndims_tot; j++) {
      update_cube_edata(vtx1, j, edata, vdata, comm_graph, node2vtx, vtx2node,
                        nsets_tot, &best_desire, imax, desire_ptr);
      update_cube_edata(vtx2, j, edata, vdata, comm_graph, node2vtx, vtx2node,
                        nsets_tot, &best_desire, imax, desire_ptr);
    }
