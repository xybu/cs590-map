 ...
 */
int make_comm_graph(pcomm_graph, graph, nvtxs, using_ewgts, assign,
                    nsets_tot) struct vtx_data **
    *pcomm_graph;        /* graph for communication requirements */
...
int nsets_tot;           /* total number of sets */
{
  ...
  *pcomm_graph = NULL;
  ...
  for (i = 0; i < nsets_tot; i++) sizes[i] = 0;
  for (i = 1; i <= nvtxs; i++) ++(sizes[assign[i]]);

  /* Now make sizes reflect the start index for each set. */
  for (i = 1; i < nsets_tot - 1; i++) sizes[i] += sizes[i - 1];
  for (i = nsets_tot - 1; i; i--) sizes[i] = sizes[i - 1];
  sizes[0] = 0;
  ...
  for (set = 0; set < nsets_tot; set++) {
    for (i = 0; i < nsets_tot; i++) adj_sets[i] = 0;
  ...
  float_ewgts =
      (float *)smalloc_ret((unsigned)(ncomm_edges + 1) * sizeof(float));
