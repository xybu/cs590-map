...
double func2d(coeffs,
              theta) double coeffs[5]; /* five different 4-way products */
...
/* Returns 1st derivative of penalty function at given angle. */
{
  ...
  val +=
      4 * (s2 * s * c - cos2 * c * s) * (coeffs[0] + coeffs[4] - 6 * coeffs[2]);

  return (val);
}

double hess2d(coeffs) double coeffs[5]; /* five different 4-way products */
