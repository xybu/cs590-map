...
void sequence(graph, nvtxs, nedges, using_ewgts, vwsqrt, solver_flag, rqi_flag,
              vmax, eigtol) struct vtx_data **graph; /* graph data structure */
...
double eigtol;   /* tolerance on eigenvectors */
{
  ...
  if (ncomps > 1) {
    ...
    for (comp = 0; comp < ncomps; comp++) setsize[comp] = 0;
    for (i = 1; i <= nvtxs; i++) ++setsize[compnum[i]];
    maxsize = 0;
    for (comp = 0; comp < ncomps; comp++)
      if (setsize[comp] > maxsize) maxsize = setsize[comp];
  ...
  for (comp = 0; comp < ncomps; comp++) {
    ...
    if (using_vwgts) {
      ...
      for (i = 1; i <= subnvtxs; i++) {
        if (subgraph[i]->vwgt > subvwgt_max) subvwgt_max = subgraph[i]->vwgt;
    ...
    if (subnvtxs == 1) {
      ...
      eigensolve(subgraph, subnvtxs, subnedges, maxdeg, subvwgt_max, subvwsqrt,
                 using_vwgts, using_ewgts, term_wgts, 0, (float **)NULL, yvecs,
                 evals, 0, (short *)space, goal, solver_flag, rqi_flag, vmax, 1,
                 3, eigtol);
  ...
  if (ncomps > 1) {
    ...
    sfree((char *)glob2loc);
    sfree((char *)setsize);
    if (using_vwgts) sfree((char *)subvwsqrt);
  }
