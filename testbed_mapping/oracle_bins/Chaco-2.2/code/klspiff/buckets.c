...
void bucketsorts(
    graph, nvtxs, buckets, listspace, dvals, sets, term_wgts, maxdval, nsets,
    parity, hops, bspace, list_length, npass,
...
int using_ewgts;           /* are edge weights being used? */
{
  ...
  if ((KL_UNDO_LIST && list_length == nvtxs) || (!KL_UNDO_LIST && !KL_RANDOM)) {
    /* Don't need to reoder if about to randomize. */
    bsptr = bspace;
    list_length = nvtxs;
    if (parity)
      for (i = 1; i <= nvtxs; i++) *bsptr++ = i;
    else
      for (i = nvtxs; i; i--) *bsptr++ = i;
  }
  ...
  for (i = 0; i < list_length; i++) { /* Loop through vertices. */
    ...
    for (j = graph[vtx]->nedges - 1; j; j--) {
      set = sets[*(++edges)];
      if (set < 0) set = -set - 1;
      if (using_ewgts) weight = *(++ewptr) * cut_cost + .5;
    ...
    for (newset = 0; newset < nsets; newset++) {
      if (newset != myset) {
        lptr = listspace[l];
        add2bilist(&lptr[vtx],
                   &buckets[myset][newset][dvals[vtx][l] + maxdval]);
        ++l;
      }
    }
  }
