...
void make_subgraph(
    graph, subgraph, subnvtxs, psubnedges, assignment, set, glob2loc, loc2glob,
    degree, using_ewgts) struct vtx_data **graph; /* graph data structure */
...
void remake_graph(
    subgraph, subnvtxs, loc2glob, degree,
    using_ewgts) struct vtx_data **subgraph; /* subgraph data structure */
int subnvtxs;                                /* number of vtxs in subgraph */
int *loc2glob;   /* mapping from subgraph to graph numbering */
short *degree;   /* degrees of vertices in graph */
int using_ewgts; /* are edge weights being used? */
