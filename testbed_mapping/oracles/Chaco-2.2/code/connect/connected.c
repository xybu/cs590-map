...
int using_ewgts;             /* are edges of graph weighted? */
{
  ...
  if (nadded == 0) {
    ...
    *cdata = (struct connect_data *)smalloc(sizeof(struct connect_data));
    ...
    add_edges(graph, new_edges, &(*cdata)->old_edges, &(*cdata)->old_ewgts,
              using_ewgts);
    *nedges += nadded;
...
void print_connected(
    cdata) struct connect_data *cdata; /* space for connectivity data */
{
  struct edgeslist *edges; /* loops through new edges */
...
void free_edgeslist(
    edge_list) struct edgeslist *edge_list; /* list to be freed */
{
