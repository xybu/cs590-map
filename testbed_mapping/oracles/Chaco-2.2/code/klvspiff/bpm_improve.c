...
void bpm_improve(graph, sets, goal, max_dev, bndy_list, weights,
                 using_vwgts) struct vtx_data *
    *graph;      /* list of graph info for each vertex */
...
int using_vwgts; /* invoke weighted cover routines? */
{
  ...
  if (DEBUG_COVER > 1) {
    printf(
        "Before first matching, sep_size = %d, sep_weight = %d,  Sizes = "
        "%g/%g\n",
  ...
  old_cost =
      sep_cost(weights[0], weights[1], (double)sep_weight, (double)max_dev);
  ...
  while (change) {
    ...
    if (DEBUG_COVER) {
      printf(
          "After big matching, sep_size = %d, sep_weight = %d,  Sizes = "
          "%g/%g\n",
          sep_size, sep_weight, weights[0], weights[1]);
    }
    if (VERTEX_COVER == 1) break;

    if (!change) {
      /* If balanced, try the other direction. */
      if (imbalance < max_dev) {
        change = bpm_improve1(graph, sets, bndy_list, weights, set_small,
                              set_big, goal, max_dev, &imbalance, &sep_size,
                              &sep_weight, using_vwgts, &old_cost);
  ...
  if (DEBUG_COVER) {
    printf(
        "After all matchings, sep_size = %d, sep_weight = %d,  Sizes = "
        "%g/%g\n\n",
        sep_size, sep_weight, weights[0], weights[1]);
  }
}
...
static int bpm_improve1(
    graph, sets, pbndy_list, weights, set_match, set_other, goal, max_dev,
    pimbalance, sep_size, sep_weight, using_vwgts,
    pcost) struct vtx_data **graph; /* list of graph info for each vertex */
short *sets;                        /* local partitioning of vtxs */
...
double *pcost;                      /* cost of current separator */
{
  ...
  if (!using_vwgts) {
    ...
    *sep_weight = *sep_size;
    ...
    wbpcover(nleft, nright, pointers, indices, vweight, sep_size, sep_weight,
             &new_bndy_list);
  }
  ...
  if (DEBUG_COVER > 1) {
    printf("Sides %.0f, %.0f: sep %d total %.0f %.0f\n", new_weights[0],
           new_weights[1], *sep_size, new_weights[0] + new_weights[1],
...
static double sep_cost(size1, size2, size_sep, max_dev) double size1,
    size2;       /* vertex weight in two partitions */
