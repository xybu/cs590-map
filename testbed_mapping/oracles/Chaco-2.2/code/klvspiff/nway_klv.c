...
int nway_klv(graph, nvtxs, lbuckets, rbuckets, llistspace, rlistspace, ldvals,
             rdvals, sets, maxdval, goal, max_dev, bndy_list,
             weightsum) struct vtx_data **graph; /* data structure for graph */
...
double *weightsum;         /* sum of vweights in each set (in and out) */
{
  ...
  while (!done) {
    ...
    time = seconds();
    bucketsortsv(graph, nvtxs, lbuckets, rbuckets, llistspace, rlistspace,
    ...
    for (step = 1;; step++) {
      ...
      if (ltop >= 0 && !left_too_big) {
        lvtx =
            ((long)lbuckets[ltop] - (long)llistspace) / sizeof(struct bilist);
      ...
      if (rtop >= 0 && !right_too_big) {
        rvtx =
            ((long)rbuckets[rtop] - (long)rlistspace) / sizeof(struct bilist);
      ...
      if (DEBUG_KL > 1) {
        printf(
            "At KLV step %d, bestvtx=%d, bestval=%d (2->%d), wt0 = %g, wt1 = "
            "%g\n",
      ...
      if (bestval < 0) {
        if (!enforce_balance || ever_balanced) neg_steps++;
        if (bestg != bestg_min)
      ...
      for (j = graph[bestvtx]->nedges - 1; j; j--) {
        ...
        if (group == 2) { /* In separator. */
          ...
          if (gain >= 0) {
            ...
            /*
                                    printf("\n  After removing %d\n", neighbor);
                                    p1bucket(from_buckets, from_listspace,
               maxdval);
            */
          ...
          for (k = graph[neighbor]->nedges - 1; k; k--) {
            ...
            if (group2 == 2) {
              ...
              if (dval >= 0) {
                movebilist(&to_listspace[neighbor2], &to_buckets[dval],
                           &to_buckets[dval + vwgt]);
                /*
                                                printf("\n  After moving %d from
                   bucket %d to bucket %d\n", neighbor2, dval, dval + vwgt);
                                                p1bucket(to_buckets,
                   to_listspace, maxdval);
                */
    ...
    /*
            printf("\nAfter pass of KLV: sets = %d/%d, sep = %d  (bestg =
       %g)\n\n\n",
                   j, k, gain, bestg);
    */
  }
