...
int give_up;                 /* has coarsening bogged down? */
{
  ...
  if (nvtxs <= vmax || give_up) {
    ...
    /*
            if (using_vwgts) {
                vwsqrt = (double *) smalloc((unsigned) (nvtxs + 1) *
       sizeof(double));
    ...
    /*
            for (i = 1; i <= ndims; i++) {
                yvecs[i] = (double *) smalloc((unsigned) (nvtxs + 1) *
       sizeof(double));
            }
    ...
    /*
            ...
            eigensolve(graph, nvtxs, nedges, maxdeg, vwgt_max, vwsqrt,
                       using_vwgts, using_ewgts, real_term_wgts, 0, (float **)
       NULL,
    ...
    /*
       ...
       (vwgt_max != 1));
            ...
            if (COARSE_KL_BOTTOM || !(step % nstep)) {
                ...
                if (COARSE_BPM) {
                    bpm_improve(graph, assignment, real_goal, max_dev,
       &bndy_list,
                                weights, using_vwgts);
                }
                if (LIMIT_KL_EWGTS)
    ...
    */
    ...
    max_dev = (step == 0) ? vwgt_max : 5 * vwgt_max;
    goal_weight = 0;
    for (i = 0; i < nsets; i++) goal_weight += real_goal[i];
    goal_weight *= KL_IMBALANCE / nsets;
    if (goal_weight > max_dev) max_dev = goal_weight;

    if (COARSE_KLV) {
      klvspiff(graph, nvtxs, assignment, real_goal, max_dev, &bndy_list,
               weights);
    }
    if (COARSE_BPM) {
      bpm_improve(graph, assignment, real_goal, max_dev, &bndy_list, weights,
                  using_vwgts);
    }
    /* */
  ...
  if (!flattened) {
    coarsen1(graph, nvtxs, nedges, &cgraph, &cnvtxs, &cnedges, &v2cv, igeom,
             coords, ccoords, using_ewgts && COARSEN_EWGTS);
  }

  if (term_wgts[1] != NULL) {
    twptr =
        (float *)smalloc((unsigned)(cnvtxs + 1) * (nsets - 1) * sizeof(float));
    twptr_save = twptr;
  ...
  if (nvtxs * COARSEN_RATIO_MIN < cnvtxs && cnvtxs > vmax && !flattened) {
    printf(
        "WARNING: Coarsening not making enough progress, nvtxs = %d, cnvtxs = "
        "%d.\n",
        nvtxs, cnvtxs);
    printf("         Recursive coarsening being stopped prematurely.\n");
    if (Output_File != NULL) {
      fprintf(Output_File,
              "WARNING: Coarsening not making enough progress, nvtxs = %d, "
              "cnvtxs = %d.\n",
              nvtxs, cnvtxs);
      fprintf(Output_File,
  ...
  coarsen_klv(cgraph, cnvtxs, cnedges, COARSEN_VWGTS, COARSEN_EWGTS, cterm_wgts,
              ...
              solver_flag, ndims, nsets, vmax, mediantype, mkconnected, eigtol,
              nstep, nextstep, &cbndy_list, weights, give_up);
  ...
  list_length = 0;
  for (i = 1; i <= nvtxs; i++) {
    if (flag[v2cv[i]]) bndy_list[list_length++] = i;
  }
  ...
  if (cterm_wgts[1] != NULL) sfree((char *)cterm_wgts[1]);
  free_graph(cgraph);
  sfree((char *)v2cv);
  ...
  if (!(step % nstep) && !flattened) {
    ...
    */
    if (!COARSEN_VWGTS && step != 0) { /* Construct new goal */
      goal_weight = 0;
      for (i = 0; i < nsets; i++) goal_weight += goal[i];
      for (i = 0; i < nsets; i++) new_goal[i] = goal[i] * (nvtxs / goal_weight);
      real_goal = new_goal;
    } else
      real_goal = goal;
    ...
    if (!using_ewgts && term_wgts[1] != NULL && step != 0) {
      twptr =
          (float *)smalloc((unsigned)(nvtxs + 1) * (nsets - 1) * sizeof(float));
      twptr_save = twptr;
      for (j = 1; j < nsets; j++) {
        new_term_wgts[j] = twptr;
        twptr += nvtxs + 1;
    ...
    */
    ...
    if (LIMIT_KL_EWGTS) restore_ewgts(graph, nvtxs);
  }
...
void print_sep_size(list, graph) int *list;
struct vtx_data **graph; /* array of vtx data for graph */
{
