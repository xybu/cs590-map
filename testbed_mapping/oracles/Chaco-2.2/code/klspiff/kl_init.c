...
int kl_init(bucket_ptrs, listspace, dvals, tops, nvtxs, nsets,
            maxchange) struct bilist ****
    *bucket_ptrs;           /* space for multiple bucket sorts */
...
int maxchange;              /* maximum change by moving a vertex */
{
  ...
  *bucket_ptrs = (struct bilist ****)array_alloc_2D_ret(
      nsets, nsets, sizeof(struct bilist *));
  ...
  *listspace = (struct bilist **)smalloc_ret((unsigned)(nsets - 1) *
