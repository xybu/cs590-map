...
void rec_median_1(
    graph, vals, nvtxs, active, cube_or_mesh, nsets, goal, using_vwgts, assign,
    top) struct vtx_data **graph; /* data structure with vertex weights */
...
int top;                          /* is this the top call in the recursion? */
{
  ...
  if (!cube_or_mesh) {
    ...
    for (i = 0; i < (nsets + 1) / 2; i++) merged_goal[0] += goal[i];
    for (i = (nsets + 1) / 2; i < nsets; i++) merged_goal[1] += goal[i];
  }
  ...
  if (nsets > 2) {
    ...
    if (!using_vwgts)
      ...
      sub_graph = (struct vtx_data **)smalloc((unsigned)(maxsize + 1) *
    ...
    for (i = 0; i < 2; i++) {
      ...
      for (j = 1; j <= sub_nvtxs; j++) sub_assign[j] = 0;
      ...
      if (sub_nsets > 1) {
        ...
        mesh_dims[0] = nsets;
        make_subgoal(goal, merged_goal, 2, cube_or_mesh, nsets, mesh_dims, i,
                     sub_vwgt_sum);

        rec_median_1(sub_graph, sub_vals, sub_nvtxs, active, cube_or_mesh,
                     sub_nsets, merged_goal, using_vwgts, sub_assign, FALSE);
      }
    ...
    if (top) {
      ndims = 0;
      for (i = 1; i < nsets; i <<= 1) ndims++;

      for (i = 0; i < nsets; i++) mapping[i] = bit_reverse(i, ndims);

      for (i = 1; i <= nvtxs; i++) assign[i] = mapping[assign[i]];
    }
...
void rec_median_k(
    graph, vals, nvtxs, active, ndims, cube_or_mesh, goal, using_vwgts,
    assign) struct vtx_data **graph; /* data structure with vertex weights */
double **vals;                       /* values of which to find median */
int nvtxs;                           /* number of values I own */
int *active;                         /* space for list of nmyvals ints */
  ...
  if (ndims > 1) {
    ...
    for (i = 0; i < 2; i++) {
      ...
      rec_median_k(sub_graph, sub_vals, sub_nvtxs, active, ndims - 1,
                   cube_or_mesh, merged_goal, using_vwgts, sub_assign);
