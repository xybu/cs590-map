...
void mapper(
    graph, xvecs, nvtxs, active, sets, ndims, cube_or_mesh, nsets, mediantype,
    goal,
...
int vwgt_max;     /* largest vertex weight */
{
  ...
  if (mediantype == 0) { /* Divide at zero instead of median. */
    ...
    for (i = 1; i <= ndims; i++) {
      ...
      vweight = 1;
      for (j = 1; j <= nvtxs; j++) {
        if (using_vwgts) vweight = graph[j]->vwgt;
        if (xvecs[i][j] < 0)
          wbelow += vweight;
        else if (xvecs[i][j] > 0)
          wabove += vweight;
  ...
  if (mediantype == 4) { /* Stripe the domain. */
    rec_median_1(graph, xvecs[1], nvtxs, active, cube_or_mesh, nsets, goal,
