...
void update_mesh_edata(
    vertex, dim, edata, vdata, comm_graph, mesh_dims, node2vtx, vtx2node,
    best_desire, imax,
    desire_ptr) int vertex;       /* graph vertex being worked on */
...
struct refine_edata **desire_ptr; /* buckets for desire values */
{
  ...
  for (i = 0; i < 2; i++) { /* Have to adjust two edges. */
    ...
    if (eguy != NULL) {
      ...
      new_desire =
          compute_mesh_edata(eguy, vdata, mesh_dims, comm_graph, node2vtx);
      ...
      if (new_desire != old_desire) { /* Update linked list if necessary. */
        ...
        if (eguy->next != NULL) eguy->next->prev = eguy->prev;
