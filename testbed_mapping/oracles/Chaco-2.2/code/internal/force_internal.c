...
void force_internal(
    graph, nvtxs, using_ewgts, assign, goal, nsets_tot,
    npasses_max) struct vtx_data **graph; /* graph data structure */
...
int npasses_max;                          /* number of passes to make */
{
  ...
  int_list = (struct bidint *)smalloc_ret((unsigned)(nsets_tot + 1) *
                                          sizeof(struct bidint));
  ...
  set_list =
      (struct bidint *)smalloc_ret((unsigned)nsets_tot * sizeof(struct bidint));
  vtx_elems = (struct bidint *)smalloc_ret((unsigned)(nvtxs + 1) *
                                           sizeof(struct bidint));
  ...
  while (any_change && npasses <= npasses_max) {
    ...
    while (progress) {
      ...
      progress = improve_internal(graph, nvtxs, assign, goal, int_list,
                                  set_list, vtx_elems, set, locked, &nlocked,
                                  using_ewgts, vwgt_max, total_vwgt);
