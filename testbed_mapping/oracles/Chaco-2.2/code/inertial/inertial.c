...
void inertial(graph, nvtxs, cube_or_mesh, nsets, igeom, coords, sets, goal,
              using_vwgts) struct vtx_data **graph; /* graph data structure */
...
int using_vwgts;  /* are vertex weights being used? */
{
  ...
  if (igeom == 1)
    inertial1d(graph, nvtxs, cube_or_mesh, nsets, coords[0], sets, goal,
               using_vwgts);

  else if (igeom == 2)
    inertial2d(graph, nvtxs, cube_or_mesh, nsets, coords[0], coords[1], sets,
               goal, using_vwgts);

  else if (igeom == 3)
    inertial3d(graph, nvtxs, cube_or_mesh, nsets, coords[0], coords[1],
               coords[2], sets, goal, using_vwgts);
