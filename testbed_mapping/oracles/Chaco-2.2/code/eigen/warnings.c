...
void warnings(workn, A, y, n, lambda, vwsqrt, Ares, bound, index, d, j, maxj,
              Sres_max, eigtol, u, Anorm,
              out_file) double *workn; /* work vector (1..n) */
  ...
  for (pass = 1; pass <= 2; pass++) {
    ...
    if (DEBUG_EVECS > 0 || WARNING_EVECS > 0) {
      if (LANCZOS_CONVERGENCE_MODE == 1) {
        fprintf(
            outfile,
            "Note about warnings: in partition convergence monitoring mode.\n");
      }
      for (i = 1; i <= d; i++) {
    ...
    if (WARNING_EVECS > 0) {
      ...
      if (warning2 && !warning3) {
        fprintf(outfile,
                "WARNING: Minor loss of orthogonality (Ares/est. > %g).\n",
                WARNING_ORTHTOL);
      }
      if (warning3) {
        fprintf(
    ...
    if (WARNING_EVECS > 2) {
      if (SRES_SWITCHES > 0) {
        fprintf(outfile,
                "WARNING: Switched routine for computing evec of T %d times.\n",
                SRES_SWITCHES);
        SRES_SWITCHES = 0;
      }
    }
    ...
    if (hosed) {
      fprintf(outfile,
              "ERROR: Sorry, out-of-bounds eigenvalue indicates serious "
              "breakdown.\n");
      fprintf(outfile,
              "       Try different parameters or another eigensolver.\n");
      if (pass == 2) bail((char *)NULL, 1);
    }
