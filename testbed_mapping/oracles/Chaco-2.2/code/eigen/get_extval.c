...
void get_extval(alpha, beta, j, ritzval, s, eigtol, wnorm_g, sigma, extval, v,
                work1, work2) double
    *alpha;     /* j-vector of Lanczos scalars (using elements 1 to j) */
...
double *work2;  /* j-vector of workspace */
{
  ...
  while (diff > tol) {
    ...
    norm_v = norm(v, 1, j);
    if (DEBUG_EVECS > 3) {
      printf("%2i   %18.16f  %18.16f  %18.16f  %g\n", cnt++, lambda_low, lambda,
             lambda_high, norm_v);
    }
    if (norm_v <= sigma) lambda_low = lambda;
    if (norm_v >= sigma) lambda_high = lambda;
