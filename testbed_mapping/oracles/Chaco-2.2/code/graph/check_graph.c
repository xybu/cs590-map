...
int check_graph(graph, nvtxs,
                nedges) struct vtx_data **graph; /* graph data structure */
    ...
    for (j = 1; j < graph[i]->nedges; j++) {
      ...
      if (!is_an_edge(graph[neighbor], i, &eweight)) {
        ...
        printf("Weight of (%d,%d)=%g, but weight of (%d,%d)=%g\n", i, neighbor,
               graph[i]->ewgts[j], neighbor, i, eweight);
        flag = TRUE;
      }
    }
...
int is_an_edge(vertex, v2,
               weight2) struct vtx_data *vertex; /* data for a vertex */
