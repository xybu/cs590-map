...
void graph_out(graph, nvtxs, using_ewgts, tag,
               file_name) struct vtx_data **graph; /* graph data structure */
...
char *file_name; /* output file name if not null */
{
  ...
  if (tag != NULL) fprintf(file, "%% graph_out: %s\n", tag);
  ...
  for (i = 1; i <= nvtxs; i++) {
    ...
    for (j = 1; j < graph[i]->nedges; j++) {
      ...
      if (using_ewgts) fprintf(file, " %.9f ", graph[i]->ewgts[j]);
    }
    fprintf(file, "\n");
  }
