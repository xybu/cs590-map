...
int define_submeshes(
    nsets, cube_or_mesh, mesh_dims, set, set_info, subsets, inert, striping,
    dir, hop_mtx_special) int nsets; /* number of subsets in this partition */
...
short hop_mtx_special[MAXSETS][MAXSETS]; /* hops values if unusual */
{
  ...
  *striping = (i <= 1 || nsets == 3 ||
  ...
  for (k = nsets - 1; k >= 0;
       k--) { /* Backwards to not overwrite current set. */
    ...
    if (*striping) { /* Use longest direction for all cuts. */
      ...
      width[dir[0]] =
          ((k + 1) * dims[dir[0]] + nsets - 1) / nsets - start[dir[0]];
    }
    ...
    for (i = 0; i < 3; i++) start[i] += set->low[i];
