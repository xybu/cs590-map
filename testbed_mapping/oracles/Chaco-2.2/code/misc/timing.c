...
void time_out(outfile) FILE *outfile; /* file to print output to */
{
  ...
  for (i = 0; i < 2; i++) {
    ...
    if (OUTPUT_TIME > 0) {
      if (total_time != 0) {
        fprintf(tempfile, "\nTotal time: %g sec.\n", total_time);
        if (input_time != 0) fprintf(tempfile, "  input %g\n", input_time);
        if (reformat_time != 0)
        ...
        if (count_time != 0) fprintf(tempfile, "  evaluation %g\n", count_time);
        if (print_assign_time != 0)
          fprintf(tempfile, "  printing assignment file %g\n",
                  print_assign_time);

        if (sim_time != 0) fprintf(tempfile, "  simulating %g\n", sim_time);
    ...
    if (OUTPUT_TIME > 1) {
      ...
      if (coarsen_time != 0 && rqi_symmlq_time == 0) {
        fprintf(tempfile, "\nCoarsening %g sec.\n", coarsen_time);
        if (match_time != 0) fprintf(tempfile, "  maxmatch %g\n", match_time);
      ...
      if (lanczos_time != 0) {
        ...
        if (blas_time != 0) fprintf(tempfile, "  vector ops %g\n", blas_time);
        if (evec_time != 0)
        ...
        if (scan_time != 0) fprintf(tempfile, "  scan %g\n", scan_time);
        if (debug_time != 0)
          fprintf(tempfile, "  debug/warning/check %g\n", debug_time);
        if (ql_time != 0) fprintf(tempfile, "  ql/bisection %g\n", ql_time);
        if (tevec_time != 0) fprintf(tempfile, "  tevec %g\n", tevec_time);
        if (ritz_time != 0) fprintf(tempfile, "  compute ritz %g\n", ritz_time);
        if (pause_time != 0) fprintf(tempfile, "  pause %g\n", pause_time);
        other_time = lanczos_time - splarax_time - orthog_time - ql_time -
                     tevec_time - ritz_time - evec_time - blas_time -
                     init_time - scan_time - debug_time - pause_time;
      ...
      if (rqi_symmlq_time != 0) {
        ...
        other_time =
            rqi_symmlq_time - coarsen_time - refine_time - lanczos_time;
        if (other_time > time_tol)
          fprintf(tempfile, "  other %g\n", other_time);
      }
    }
