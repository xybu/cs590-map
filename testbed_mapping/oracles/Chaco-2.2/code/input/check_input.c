...
int check_input(
    graph, nvtxs, nedges, igeom, coords, graphname, assignment, goal,
    architecture, ndims_tot, mesh_dims, global_method, local_method, rqi_flag,
    vmax, ndims,
    eigtol) struct vtx_data **graph; /* linked lists of vertex data */
...
double eigtol;     /* tolerance for eigen-pairs */
{
  ...
  if (graph != NULL) {
    ...
    if (global_method == 1 || global_method == 2 || local_method == 1) {
      printf(
          "No graph input.  Only allowed for inertial or simple methods "
          "without KL.\n");
      flag_graph = TRUE;
    }
  }
  ...
  if (architecture < 0 || architecture > 3) {
    ...
    if (architecture == 2 &&
        (mesh_dims[0] <= 0 || mesh_dims[1] <= 0 || mesh_dims[2] <= 0)) {
      printf("Size of 3-D mesh improperly specified, %dx%dx%d.\n", mesh_dims[0],
             mesh_dims[1], mesh_dims[2]);
      flag = TRUE;
    }
  }
  ...
  if (global_method == 1 || (global_method == 2 && rqi_flag)) {
    ...
    if (*vmax < i) {
      printf(
          "WARNING: Number of vertices in coarse graph (%d) being reset to "
          "%d.\n",
          *vmax, i);
      if (Output_File != NULL) {
        fprintf(Output_File,
                "WARNING: Number of vertices in coarse graph (%d) being reset "
                "to %d.\n",
                *vmax, i);
      }
      *vmax = i;
  ...
  if (global_method == 7 && local_method == 1) {
    if (nprocs > 1 << ndims) {
      printf(
          "Can only use local method on single level of read-in assignment,\n");
  ...
  if (fabs(vwgt_sum - vwgt_sum2) > 1e-5 * (vwgt_sum + vwgt_sum2)) {
    printf(
        "Sum of values in goal (%g) not equal to sum of vertex weights (%g).\n",
        vwgt_sum2, vwgt_sum);
    flag = TRUE;
  }
  ...
  if (global_method == 7 && !flag) {
    flag_assign =
        check_assignment(assignment, nvtxs, nprocs, ndims, local_method);
  }
...
static int check_params(
    global_method, local_method, rqi_flag,
    ndims) int global_method; /* global partitioning algorithm */
int local_method;             /* local partitioning algorithm */
...
int ndims;                    /* number of eigenvectors */
{
  ...
  if (global_method == 1 || global_method == 2) {
    ...
    if (global_method == 2 && rqi_flag) {
      ...
      if (TERM_PROP) {
        printf(
            "WARNING: Using default Lanczos for extended eigenproblem, not "
            "RQI/Symmlq.\n");
      }
    }
  ...
  if (TERM_PROP) {
    ...
    if (ndims > 1) {
      printf(
          "WARNING: May ignore terminal propagation in spectral quadri/octa "
          "section\n");
    }
  }
...
static int check_assignment(
    assignment, nvtxs, nsets_tot, ndims,
    local_method) short *assignment; /* set numbers if read-from-file */
int nvtxs;                           /* number of vertices */
