...
int       main()
{
    ...
    short    *assignment;	/* set number of each vtx (length nvtxs+1 ----------- NOT REAL! actual length = nvtxs!) */
    int      *host_percent; /* host percentage of each vtx (length nvtxs) ------------------------------ADD! */
	...
	int  	  set_capa_func[20][3];	/* capacity function of each PM (up to 20 PMs)-------------ADD! */
	...
	input_queries(&fin, &fin_host, &fingeom, &finassign, graphname, hostname, geomname, inassignname,
		      outassignname, outfilename,
		      &architecture, &ndims_tot, &set_capa, set_capa_func, usage, mesh_dims,
		      &global_method, &local_method, &rqi_flag, &vmax, &ndims); //ADD!
	...
	flag = input_graph(fin, graphname, &start, &adjacency, &nvtxs, &vwgts, &ewgts);  //ADD!
	if (flag) {
	    sfree((char *) ewgts);
	    sfree((char *) vwgts);
	...
	interface(nvtxs, start, adjacency, vwgts, ewgts, x, y, z,
		  ...
		  assignment,
		  architecture, ndims_tot, mesh_dims, set_capa, goal,
	...
	int sati = 1;
	for (cur = 0; cur < nprocs; cur++) {
		printf("\tcurrent host percentage after partitioning %d: %d\t old switch usage percentage: %d\n", cur, host_usage[cur], usage[cur]);
