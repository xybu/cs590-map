...
double   *dist;			/* distances defining splitter. Siyuan: Determine how we partition the sets (Very important!!) */
...
int       vwgt_max;		/* largest vertex weight */
{
	...
	for (i = 0; i < nsets; i++)
	    active[i] = FALSE;
	active[badset] = TRUE;  /* Siyuan: Only the bad set (most imbalanced set) is active */

	done = FALSE;
	while (!done) {
	    nextmove(nvtxs, nsets, vals, indices, startvtx, dist, sets,
		     toobig, active, &vtx, &to, &delta); // Siyuan: Here we get which vertax to move and where to move
	    ...
	    for (i = 0; i < nsets; i++)
		if (active[i]) //Siyuan: Don't just use active[badset],because the array "active" has been changed...
		    dist[i] -= toobig * delta;
