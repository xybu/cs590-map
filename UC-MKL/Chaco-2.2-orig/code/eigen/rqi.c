...
void rqi(A, yvecs, index, n, r1, r2, v, w, x, y, work, tol, initshift, evalest,
         ...
         goal, vwgt_max,
         ndims) struct vtx_data **A; /* matrix/graph being analyzed */
...
int ndims;                 /* dimensionality of partition */
{
  ...
  if (DEBUG_EVECS > 1) {
    printf(
        "  step      lambda est.            Ares          Symmlq its.   istop  "
        "factor  delta\n");
    printf("    0");
    doubleout(shift, 1);
    doubleout(res, 1);
  ...
  while (!converged) {
    ...
    symmlq_(&long_n, &u[1], &r1[1], &r2[1], &v[1], &w[1], &x[1], &y[1], work,
            &checka, &goodb, &precon, &shift, &nout, &intlim, &rtol, &istop,
            &itn, &anorm, &acond, &rnorm, &ynorm, (double *)A, vwsqrt,
            (double *)orthlist, &macheps, &normxlim, &itnmin);
    symmlqitns += itn;
    normx = norm(x, 1, n);
    ...
    if (RQI_CONVERGENCE_MODE == 1 && !converged && ndims == 1) {
      if (first) {
        assign(A, yvecs, n, 1, cube_or_mesh, nsets, vwsqrt, assignment, active,
        ...
        if (assigndiff <= assigntol) converged = TRUE;
      }
    }
  ...
  *evalest = shift;
  ...
  if (WARNING_EVECS > 0 && warning) {
    strout(
        "WARNING: Residual convergence not monotonic; RQI may have "
        "misconverged.\n");
  }
