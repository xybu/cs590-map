...
int vwgt_max;      /* largest vertex weight */
{
  ...
  struct orthlink_float *
      *solist; /* vec. of structs with vecs. to orthog. against */
  struct scanlink
  ...
  struct orthlink_float *
  makeorthlnk_float();           /* makes space for new entry in orthog. set */
  ...
  float *
  mkvec_ret_float();     /* allocates space for a vector, returns error code */
  ...
  solist = (struct orthlink_float **)smalloc((unsigned)(maxj + 1) *
                                             sizeof(struct orthlink_float *));
  ...
  while ((j <= maxj) && (!converged) && (ritzval_flag == 0) && memory_ok) {
    ...
    time = lanc_seconds();
    pause = lanpause_float(j, lastpause, interval, q, n, &pausemode, version,
                           beta[j]);
    ...
    if (pause) {
      ...
      if (DEBUG_EVECS > 2 && !converged) {
        ...
        for (i = 1; i <= d; i++) {
          ...
          Ares[i] =
              checkeig(workn_double, A, y[i], n, lambda[i], vwsqrt, u_double);
          printf("%2d.", i);
  ...
  /* Collect eigenvalue and bound information. Only compute and display info for
     the eigpairs actually used in the partitioning since don't want to spend
     the
  ...
  time = lanc_seconds();
  float_to_double(u_double, 1, n, u);
  warnings(workn_double, A, y, n, lambda, vwsqrt, Ares, bound, index, d, j,
           maxj, Sres_max, eigtol, u_double, Anorm, Output_File);
  debug_time += lanc_seconds() - time;
