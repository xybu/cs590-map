...
int bisect(alpha, beta, j, Anorm, workj, ritz, nevals_left, nevals_right, tol,
           ritz_sav, max_steps) double *alpha; /* vector of Lanczos scalars */
    ...
    while ((x2 - x1) > tol || numbracketed > 1) {
      ...
      if (ritz_sav != NULL) {
        ...
        printf("%4d %20.16f %20.16f   %3d   %3d  %3d   %3d   %3d   %3d   %g",
               ii++, x1, x2, x1cnt, x1ck, x2cnt, x2ck, numbracketed, numck,
               diff);
