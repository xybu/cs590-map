...
void coarsen(graph, nvtxs, nedges, using_vwgts, using_ewgts, term_wgts, igeom,
             coords, yvecs, ndims, solver_flag, vmax, eigtol, nstep, step,
             give_up)
...
int give_up;                 /* has coarsening bogged down? */
{
  ...
  if (nvtxs <= vmax || give_up) {
    ...
    for (i = 0; i < nsets; i++) goal[i] = total_vwgt / nsets;
    ...
    eigensolve(graph, nvtxs, nedges, maxdeg, vwgt_max, vwsqrt, using_vwgts,
               using_ewgts, real_term_wgts, igeom, coords, yvecs, evals, 0,
               space, goal, solver_flag, FALSE, 0, ndims, 3, eigtol);
  ...
  coarsen(cgraph, cnvtxs, cnedges, COARSEN_VWGTS, COARSEN_EWGTS, cterm_wgts,
          igeom, ccoords, cyvecs, ndims, solver_flag, vmax, eigtol, nstep,
          nextstep, give_up);
  ...
  if (!(step % nstep)) {
    ...
    if (using_vwgts) {
      ...
      for (i = 1; i <= ndims; i++) orthog1(yvecs[i], 1, nvtxs);
    ...
    for (i = 1; i < ndims; i++) {
      ...
      rqi(graph, yvecs, i, nvtxs, r1, r2, v, w, x, y, work, eigtol, initshift,
          &evalest, vwsqrt, orthlist, 0, nsets, space, morespace, 3, goal,
          vwgt_max, ndims);
    ...
    if (term_wgts[1] != NULL && ndims == 1) {
      ...
      /* If not coarsening ewgts, then need care with term_wgts. */
      if (!using_ewgts && term_wgts[1] != NULL && step != 0) {
        twptr = (float *)smalloc((unsigned)(nvtxs + 1) * (nsets - 1) *
                                 sizeof(float));
        twptr_save = twptr;
        for (j = 1; j < nsets; j++) {
          new_term_wgts[j] = twptr;
      ...
      rqi(graph, yvecs, ndims, nvtxs, r1, r2, v, w, x, y, work, eigtol,
          initshift, &evalest, vwsqrt, orthlist, 0, nsets, space, morespace, 3,
          goal, vwgt_max, ndims);
    }
