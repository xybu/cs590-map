...
int refine_part(graph, nvtxs, using_ewgts, assign, architecture, ndims_tot,
                mesh_dims,
                goal) struct vtx_data **graph; /* graph data structure */
...
double *goal;     /* desired set sizes */
{
  ...
  set_list =
      (struct bilist *)smalloc_ret((unsigned)nsets_tot * sizeof(struct bilist));
  vtx_elems = (struct bilist *)smalloc_ret((unsigned)(nvtxs + 1) *
  ...
  for (set = 0; set < nsets_tot - 1; set++) {
    ...
    for (ptr = set_list[set].next; ptr != NULL; ptr = ptr->next) {
      ...
      for (j = 1; j < graph[vertex]->nedges; j++) {
        ...
        if (using_ewgts) ewgt = graph[vertex]->ewgts[j];
        adj_sets[set2] += ewgt;
      }
    }
  ...
  pairs = (struct ipairs *)smalloc_ret((unsigned)(ncomm + 1) *
                                       sizeof(struct ipairs));
  ...
  subgraph = (struct vtx_data **)smalloc_ret((unsigned)(nmax + 1) *
  ...
  for (i = ncomm - 1; i >= 0; i--) {
    ...
    if (sizes[set1] + sizes[set2] > nmax) {
      ...
      temp =
          srealloc_ret((char *)degrees, (unsigned)(nmax + 1) * sizeof(short));
      if (temp == NULL) {
        goto skip;
      } else {
        degrees = (short *)temp;
      ...
      temp = srealloc_ret((char *)loc2glob, (unsigned)(nmax + 1) * sizeof(int));
      if (temp == NULL) {
        goto skip;
      } else {
        loc2glob = (int *)temp;
    ...
    change = kl_refine(graph, subgraph, set_list, vtx_elems, assign, set1, set2,
                       glob2loc, loc2glob, sub_assign, old_sub_assign, degrees,
                       using_ewgts, hops, goal, sizes, term_wgts, architecture,
                       mesh_dims);
