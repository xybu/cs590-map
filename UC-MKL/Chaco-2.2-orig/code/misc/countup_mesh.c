...
void countup_mesh(
    graph, nvtxs, assignment, mesh_dims, print_lev, outfile,
    using_ewgts) struct vtx_data **graph; /* graph data structure */
...
int using_ewgts;   /* are edge weights being used? */
{
  ...
  for (j = 0; j < nsets; j++) setsize[j] = 0;
  for (i = 1; i <= nvtxs; i++) ++setsize[assignment[i]];

  /* Modify setsize to become index into vertex list. */
  for (j = 1; j < nsets; j++) setsize[j] += setsize[j - 1];
  for (j = nsets - 1; j > 0; j--) startptr[j] = setsize[j] = setsize[j - 1];
  ...
  for (set = 0; set < nsets; set++) {
    ...
    if (setsize[set] > max_size) max_size = setsize[set];
  }
  min_size = max_size;
  for (set = 0; set < nsets; set++) {
    if (setsize[set] < min_size) min_size = setsize[set];
  }
  ...
  for (set = 0; set < nsets; set++) {
    ...
    if (bdyvtx_hops > bdyvtx_hops_max) bdyvtx_hops_max = bdyvtx_hops;
    if (set == 0 || bdyvtx_hops < bdyvtx_hops_min)
      bdyvtx_hops_min = bdyvtx_hops;
    if (internal > max_internal) max_internal = internal;
    if (set == 0 || internal < min_internal) min_internal = internal;
    ...
    if (print_lev < 0) {
      ...
      if (print2file)
        fprintf(outfile, " %5d    %5d    %6g     %6g   %6g      %6d\n", set,
                setsize[set], cutsize[set], hopsize[set], bdyvtxs,
                neighbor_sets);
    }
    if (cutsize[set] > maxcuts) maxcuts = cutsize[set];
    if (set == 0 || cutsize[set] < mincuts) mincuts = cutsize[set];
    if (hopsize[set] > maxhops) maxhops = hopsize[set];
    if (set == 0 || hopsize[set] < minhops) minhops = hopsize[set];
    if (bdyvtxs > maxbdy) maxbdy = bdyvtxs;
    if (set == 0 || bdyvtxs < minbdy) minbdy = bdyvtxs;
    if (neighbor_sets > maxneighbors) maxneighbors = neighbor_sets;
    if (set == 0 || neighbor_sets < minneighbors) minneighbors = neighbor_sets;
    ncuts += cutsize[set];
    nhops += hopsize[set];
