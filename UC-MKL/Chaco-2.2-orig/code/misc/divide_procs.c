...
int divide_procs(architecture, ndims, ndims_tot, set_info, set, subsets, inert,
                 pndims_real, pnsets_real, pstriping, cut_dirs, mesh_dims,
                 hops_special) int architecture; /* 0 => hypercube, d =>
                                                    d-dimensional mesh */
...
short hops_special[][MAXSETS]; /* hop matrix for nonstandard cases */
{
  ...
  if (architecture > 0) { /* Mesh, complicated case. */
    ...
    ndims_real = ndims;
    while (1 << ndims_real > nsets_real) --ndims_real;
    ...
    flag = define_submeshes(nsets_real, architecture, mesh_dims, set, set_info,
                            subsets, inert, &striping, cut_dirs, hops_special);
    if (striping) {
      ndims_real = 1;
    }
  }
    ...
    flag = define_subcubes(nsets_real, ndims_tot, ndims_real, set, set_info,
                           subsets, inert, &striping, hops_special);

    if (striping) ndims_real = 1;
  }
