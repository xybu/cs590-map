...
int submain(graph, nvtxs, nedges, using_vwgts, using_ewgts, igeom, coords,
            ...
            ndims_tot, mesh_dims, global_method, local_method, rqi_flag, vmax,
            ndims, eigtol,
            seed) struct vtx_data **graph; /* data structure for graph */
...
long seed;           /* for random graph mutations */
{
  ...
  if (CHECK_INPUT) { /* Check the input for inconsistencies. */
    ...
    flag =
        check_input(graph, nvtxs, nedges, igeom, coords, graphname, assignment,
                    goal, architecture, ndims_tot, mesh_dims, global_method,
                    local_method, rqi_flag, &vmax, ndims, eigtol);
  ...
  if (ECHO != 0) {
    reflect_input(nvtxs, nedges, igeom, graphname, geomname, inassignname,
                  ...
                  mesh_dims, global_method, local_method, rqi_flag, vmax, ndims,
                  eigtol, seed, outfile);
  }
  ...
  if (PRINT_HEADERS) {
    ...
    if (Output_File != NULL) {
      fprintf(Output_File,
              "\n\nStarting to partition ... (residual, warning and error "
              "messages only)\n\n");
    }
  }
  ...
  if (SEQUENCE) {
    sequence(graph, nvtxs, nedges, using_ewgts, vwsqrt, LANCZOS_TYPE, rqi_flag,
             vmax, eigtol);
    goto End_Label;
  }
  ...
  graph2 = graph;
  if (global_method == 3 && local_method != 1 && !VERTEX_COVER &&
      !using_vwgts) {
    graph2 = NULL;
  }

  if (!(global_method == 7 && local_method == 2)) {
    balance(graph2, nvtxs, nedges, using_vwgts, using_ewgts, vwsqrt, igeom,
