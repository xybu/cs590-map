...
void read_params(pfile) FILE *pfile; /* file with new user parameters */
{
  ...
  static char *TFpnames[] = {
      /* names of true/false parameters */
  ...
  static int *TFparams[] = {
      ...
      &CHECK_INPUT,    &MAKE_CONNECTED, &COARSEN_VWGTS,    &COARSEN_EWGTS,
      &KL_RANDOM,      &KL_ONLY_BNDY,   &KL_UNDO_LIST,     &COARSE_KL_BOTTOM,
      &PERTURB,        &OUTPUT_ASSIGN,  &PROMPT,           &PERCENTAGE_OUTPUT,
      &MAKE_VWGTS,     &REFINE_MAP,     &PRINT_HEADERS,    &TERM_PROP,
      &LIMIT_KL_EWGTS, &SIMULATOR,      &FREE_GRAPH,       &SEQUENCE,
      &LANCZOS_TIME,   &TIME_KERNELS,   &HEAVY_MATCH,      &EXPERT,
      &OUT_ASSIGN_INV, &IN_ASSIGN_INV,  &VERTEX_SEPARATOR, &VERTEX_COVER,
      &FLATTEN};
  ...
  static double *dparams[] = {
      /* pointers to double precision parameters */
      &EIGEN_TOLERANCE, &BISECTION_SAFETY, &PERTURB_MAX,       &WARNING_ORTHTOL,
      &WARNING_MISTOL,  &SRESTOL,          &COARSEN_RATIO_MIN, &EWGT_RATIO_MAX,
      &CUT_COST,        &HOP_COST,         &BDY_COST,          &BDY_HOP_COST,
      ...
      if (ipnames[i] != NULL) {
        ...
        if (flag) {
          ...
          if (DEBUG_PARAMS > 0) {
            ...
            if (Output_File != NULL) {
              fprintf(Output_File,
                      " ERROR reading value for `%s' in line %d parameter file "
                      "%s\n",
                      ipnames[i], linenum, PARAMS_FILENAME);
            }
          }
        ...
        if (TFpnames[i] != NULL) {
          ...
          if (flag) {
            ...
            if (DEBUG_PARAMS > 0) {
              ...
              if (Output_File != NULL) {
                fprintf(Output_File,
                        " ERROR reading value for `%s' in line %d parameter "
                        "file %s\n",
                        TFpnames[i], linenum, PARAMS_FILENAME);
              }
            }
...
static int read_intTF(ptr, val) char *ptr; /* pointer to string to parse */
...
static int read_double(ptr, val) char *ptr; /* pointer to string to parse */
double *val;                                /* value returned */
{
  int nvals; /* number of values sucessfully read */

  while (*ptr != '-' && *ptr != '.' && !isdigit(*ptr) && *ptr != '\0') ptr++;
...
static int read_long(ptr, val) char *ptr; /* pointer to string to parse */
long *val;                                /* value returned */
{
  int nvals; /* number of values sucessfully read */

  while (*ptr != '-' && !isdigit(*ptr) && *ptr != '\0') ptr++;
...
static int read_string(ptr, val) char *ptr; /* pointer to string to parse */
