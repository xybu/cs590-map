...
void find_flat(
    graph, nvtxs, pcnvtxs,
    v2cv) struct vtx_data **graph; /* data structure for storing graph */
int nvtxs;                         /* number of vertices in graph */
...
int SameStructure(node1, node2, graph, scatter) int node1,
    node2;               /* two vertices which might have same nonzeros */
...
int *scatter;            /* array for checking vertex labelling */
{
  ...
  for (i = 1; i < graph[node2]->nedges; i++) {
    if (scatter[graph[node2]->edges[i]] != node1) break;
  }
