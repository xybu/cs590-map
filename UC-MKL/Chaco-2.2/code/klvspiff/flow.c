...
void wbpcover(n_left, n_right, pointers, indices, vweight, psep_size,
              psep_weight,
              psep_nodes) int n_left; /* number of vertices on left side */
...
int **psep_nodes;                     /* list of separator nodes */
{
  ...
  if (DEBUG_COVER) {
    ...
    printf(
        "    Corresponds to unweighted, nleft = %d, nright = %d, 2*nedges = "
        "%d\n",
        wleft, wright, wedges);
  }
...
static void bpflow(n_left, n_right, pointers, indices, vweight, resid, flow,
                   touched) int n_left; /* number of vertices on left side */
int n_right;                            /* number of vertices on right side */
int *pointers;                          /* start/stop of adjacency lists */
int *indices;                           /* adjacency list for each vertex */
int *vweight;                           /* vertex weights */
...
static int augment(node, pointers, indices, resid, flow, touched,
                   seen) int node; /* start node in augmenting path */
int *pointers;                     /* start/stop of adjacency lists */
int *indices;                      /* adjacency list for each vertex */
int *resid;                        /* residual weight at each vertex */
int *flow;                         /* flow on right->left edges */
...
static void touch(node, pointers, indices, resid, flow, touched, flow1, seen,
                  nseen) int node;
int *pointers; /* start/stop of adjacency lists */
int *indices;  /* adjacency list for each vertex */
int *resid;    /* residual weight at each vertex */
        ...
        *flow1 = flow2;
        ...
        for (j = pointers[neighbor]; j < pointers[neighbor + 1]; j++) {
          if (flow[j] && !touched[indices[j]]) {
            ...
            touch(indices[j], pointers, indices, resid, flow, touched, &flow2,
                  seen, nseen);
...
static void reachability(
    n_left, n_right, pointers, indices, resid, flow,
    touched) int n_left; /* number of vertices on left side */
int n_right;             /* number of vertices on right side */
int *pointers;           /* start/stop of adjacency lists */
int *indices;            /* adjacency list for each vertex */
...
int *touched;            /* flags for each vertex */
{
  ...
  for (i = 0; i < n_left + n_right; i++) touched[i] = 0;
...
/* Mark everybody in my alternating path tree, and return vertex at */
/* end of augmenting path if found. */
static int touch2(node, pointers, indices, flow, touched) int node;
int *pointers; /* start/stop of adjacency lists */
int *indices;  /* adjacency list for each vertex */
int *flow;     /* flow on right->left edges */
int *touched;  /* flags for each vertex */
...
int count_flow(n_left, n_right, pointers, flow) int n_left;
int n_right;
int *pointers;
int *flow;
{
...
void count_resid(n_left, n_right, resid, vweight, marked) int n_left;
int n_right;
int *resid;
int *vweight;
int *marked;
...
void check_resid(n_left, n_right, vweight, resid, pointers, indices,
                 flow) int n_left;
int n_right;
int *vweight; /* vertex weights */
int *resid;
