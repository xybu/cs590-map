...
long      seed;			/* for random graph mutations */
{
	...
	if (capa_sum < vwgt_sum) {	
		...
		for (i = 0; i < nsets_tot; i++) {
			...
			int max_index = 0;
			int max_capa = 0;
			for (j = 0; j < nsets_tot; j++) { //find the index of the i'th largest capacity
				if(goal[j] < 0.1 && set_capa[j] >= max_capa) { //may have problem (<1???)
					max_index = j;
					max_capa = set_capa[j];
				}
			}
...
//printf("1111111111111   %d\n", max_index);
	...
	flag = reformat(start, adjacency, nvtxs, &nedges, vwgts, ewgts, &graph); //may have segmentation fault Siyuan!
