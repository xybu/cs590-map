...
int give_up;                 /* has coarsening bogged down? */
{
  ...
  if (nvtxs <= vmax || give_up) {
    ...
    if (mkconnected) {
      make_connected(graph, nvtxs, &nedges, (short *)&(yvecs[1][0]), active,
                     &cdata, using_ewgts);
    ...
    eigensolve(graph, nvtxs, nedges, maxdeg, vwgt_max, vwsqrt, using_vwgts,
               using_ewgts, real_term_wgts, 0, (float **)NULL, yvecs, evals,
               architecture, assignment, fake_goal, solver_flag, FALSE, 0,
               ndims, mediantype, eigtol);

    if (mkconnected) make_unconnected(graph, &nedges, &cdata, using_ewgts);

    assign(graph, yvecs, nvtxs, ndims, architecture, nsets, vwsqrt, assignment,
           active, mediantype, real_goal, vwgt_max);
    // REVISED //
    // for (i = 0; i < nvtxs; i++) printf("%d\t",assignment[i]);
    // printf("\n");
    //////////////////
    /*
    ...
    */
    ...
    if (COARSE_KL_BOTTOM || !(step % nstep)) {
      ...
      // REVISED //
      // printf("klspiff 2!!!!!!\n");
      klspiff(graph, nvtxs, assignment, nsets, hops, real_goal, real_term_wgts,
              max_dev, maxdeg, using_ewgts, &bndy_list, weights);
