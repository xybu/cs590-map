...
int nway_kl(graph, nvtxs, buckets, listspace, tops, dvals, sets, maxdval, nsets,
            goal, term_wgts, hops, max_dev, using_ewgts, bndy_list,
            startweight) struct vtx_data **graph; /* data structure for graph */
  ...
  while (!done) {
    ...
    for (step = 1;; step++) {
        ...
        for (j = 0; j < nsets; j++)
          ...
          if (i != j) {
            ...
            if (k >= 0) {
              ...
              if ((enforce_balance_hard && weightsum[i] >= goal[i] &&
                   weightsum[j] <= goal[j] &&
                   weightsum[i] - goal[i] - (weightsum[j] - goal[j]) >
                       max_dev) ||
                  (!enforce_balance_hard && weightsum[i] >= goal[i] &&
                   weightsum[j] <= goal[j]) ||
      ...
      if (DEBUG_KL > 1) {
        printf("At KL step %d, bestvtx=%d, bestval=%d (%d-> %d)\n", step,
               bestvtx, bestval, bestfrom, bestto);
      }
      ...
      *endlist = bestptr;
      ...
      if (using_ewgts) ewptr = graph[bestvtx]->ewgts;
      for (j = graph[bestvtx]->nedges - 1; j; j--) {
        neighbor = *(++edges);
        if (using_ewgts) eweight = *(++ewptr) * cut_cost + .5;
        ...
        if (sets[neighbor] >= 0) {
          ...
          for (k = 0; k < nsets; k++) {
            if (k != group) {
              diff = eweight * (hops[k][bestfrom] - hops[group][bestfrom] +
    ...
    if (!done) { /* Prepare for next pass. */
      if (KL_UNDO_LIST && worth_undoing && !balance_trouble) {
        ...
        list_length = make_kl_list(graph, movelist, buckets, listspace, sets,
                                   nsets, bspace, dvals, maxdval);
      }
    }
    ...
    if (done && *bndy_list != NULL) {
      make_bndy_list(graph, movelist, buckets, listspace, sets, nsets, bspace,
                     tops, bndy_list);
    }
  }
