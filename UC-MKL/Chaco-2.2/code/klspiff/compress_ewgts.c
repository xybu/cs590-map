...
void compress_ewgts(graph, nvtxs, nedges, ewgt_max, using_ewgts) struct vtx_data
...
int using_ewgts; /* are edge weights being used? */
{
  ...
  if (!using_ewgts)
    ...
    new_ewgts =
        (float *)smalloc((unsigned)(2 * nedges + nvtxs) * sizeof(float));
...
void restore_ewgts(
    graph,
    nvtxs) struct vtx_data **graph; /* list of graph info for each vertex */
int nvtxs;                          /* number of vertices in graph */
{
  int i; /* loop counter */
  int sfree();
