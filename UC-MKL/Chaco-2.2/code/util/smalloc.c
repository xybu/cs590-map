...
double *smalloc(n) unsigned int n; /* number of bytes to be allocated */
{
  ...
  if (n == 0) {
    ...
    if (Output_File != NULL) {
      fprintf(Output_File, "ERROR: Non-positive argument to smalloc (%u).\n",
              n);
    }
    bail((char *)NULL, 1);
  }
  ...
  if (DEBUG_MEMORY > 1) {
    new =
        (struct smalloc_debug_data *)malloc(sizeof(struct smalloc_debug_data));

    if (new == NULL) {
      printf("WARNING: No space for malloc_debug %u.\n", n);
      if (Output_File != NULL) {
        fprintf(Output_File, "WARNING: No space for malloc_debug %u.\n", n);
      }
      return (ptr);
    }
...
double *smalloc_ret(n) unsigned int n; /* number of bytes to be allocated */
{
  extern FILE *Output_File;       /* output file or null */
  extern int DEBUG_MEMORY;        /* use debug memory allocator? */
  double *ptr;                    /* return value */
  ...
  if (n == 0) {
    ...
    if (ptr == NULL) {
      ...
      if (DEBUG_MEMORY > 1) {
        new = (struct smalloc_debug_data *)malloc(
            sizeof(struct smalloc_debug_data));
...
double *srealloc(ptr, n) char *ptr; /* pointer to current location */
  ...
  if (ptr == NULL) {
    ...
    if (n == 0) {
      ...
      if (DEBUG_MEMORY > 1) {
        ...
        if (dbptr == NULL) {
          printf(
              "Memory error: In srealloc, address not found in debug list "
              "(0x%lx)\n",
              (long)ptr);
        } else {
          dbptr->size = n;
...
/* Safe version of realloc */
/* Returns instead of dying if it fails. */
double *srealloc_ret(ptr, n) char *ptr; /* pointer to current location */
unsigned int n;                         /* desired size of new allocation */
{
  extern FILE *Output_File;         /* output file or null */
  double *p;                        /* returned pointer */
  ...
  if (ptr == NULL) {
    ...
    if (n == 0) {
      ...
      if (DEBUG_MEMORY > 1) {
        ...
        if (dbptr == NULL) {
          printf(
              "Memory error: In srealloc_ret, address not found in debug list "
              "(0x%lx)\n",
              (long)ptr);
        } else {
          dbptr->size = n;
...
int sfree(ptr) char *ptr;
{
  ...
  if (DEBUG_MEMORY > 1) {
    if (ptr != NULL) { /* search through debug list for it */
      ...
      if (dbptr == NULL) {
        printf(
            "Memory error: In sfree, address not found in debug list (0x%lx)\n",
            (long)ptr);
        if (Output_File != NULL) {
          printf(
              "Memory error: In sfree, address not found in debug list "
              "(0x%lx)\n",
              (long)ptr);
        }
      } else {
