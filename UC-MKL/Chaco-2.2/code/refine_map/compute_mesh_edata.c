...
double compute_mesh_edata(
    edata, vdata, mesh_dims, comm_graph,
    node2vtx) struct refine_edata *edata; /* desire data for current edge */
...
short *node2vtx; /* maps mesh nodes to graph vertices */
{
  ...
  desire = (vdata[off + vtx1].above - vdata[off + vtx1].below -
            vdata[off + vtx1].same) +
           (vdata[off + vtx2].below - vdata[off + vtx2].above -
            vdata[off + vtx2].same);

  /* Subtract off potential doubly counted edge. */
  if (is_an_edge(comm_graph[vtx1], vtx2, &ewgt)) desire -= 2 * ewgt;
