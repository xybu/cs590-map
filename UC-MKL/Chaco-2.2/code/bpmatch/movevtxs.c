...
void movevtxs(
    graph, nvtxs, nsets, dist, indices, vals, startvtx, sets, size, goal,
    vwgt_max) struct vtx_data **graph; /* data structure with vertex weights */
int nvtxs;                             /* number of vertices in graph */
int nsets;                             /* how many sets am I dividing into? */
double *dist; /* distances defining splitter. Siyuan: Determine how we partition
                 the sets (Very important!!) */
...
int vwgt_max;            /* largest vertex weight */
{
  ...
  while (!balanced) {
    ...
    active[badset] =
        TRUE; /* Siyuan: Only the bad set (most imbalanced set) is active */

    done = FALSE;
    while (!done) {
      nextmove(nvtxs, nsets, vals, indices, startvtx, dist, sets, toobig,
               active, &vtx, &to, &delta);  // Siyuan: Here we get which vertax
                                            // to move and where to move
      from = sets[vtx];  // Siyuan: Now vtx is the vertax needed to be moved
      weight = graph[vtx]->vwgt;

      /* Now adjust all active dists to reflect this move so far. */
      for (i = 0; i < nsets; i++)
        if (active[i])  // Siyuan: Don't just use active[badset],because the
                        // array "active" has been changed...
          dist[i] -= toobig * delta;
...
static void nextmove(nvtxs, nsets, vals, indices, startvtx, dist, sets, toobig,
                     active, next_vtx, next_to,
                     next_delta) int nvtxs; /* number of vertices in graph */
int nsets;               /* how many sets am I dividing into? */
...
static void couple(nsets, from, to,
                   vtx) int nsets; /* number of sets being divided into */
...
static void undo_coupling(
    graph, sets, nsets, from, to, toobig, badset,
    size) struct vtx_data **graph; /* data structure with vertex weights */
    ...
    while (!done) {
      ...
      for (i = 0; i < ncoupled && !found; i++) {
        if (coupled_sets[i][from] == 1) { /* Found an edge into set. */
          ...
          for (j = 0; j < nsets; j++) {
            if (coupled_sets[i][j] == -1) from = (short)j;
          }
    ...
    while (!done) {
      ...
      for (i = 0; i < ncoupled && !found; i++) {
        if (coupled_sets[i][from] == -1) { /* Found an edge from set. */
          ...
          /* And find other end of edge. */
          for (j = 0; j < nsets; j++) {
            if (coupled_sets[i][j] == 1) to = (short)j;
          }
