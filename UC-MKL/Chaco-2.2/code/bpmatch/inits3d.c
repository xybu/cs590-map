...
void inits3d(
    graph, xvecs, vals, indices, nvtxs, dist, startvtx, size,
    sets) struct vtx_data **graph; /* graph data structure for vertex weights */
...
short *sets;                       /* set each vertex gets assigned to */
{
  ...
  */
  ...
  startvtx[0][7] =
      findindex(indices[0][7], vals[0][7], dist[7] - dist[0], nvtxs);
  startvtx[1][6] =
      findindex(indices[1][6], vals[1][6], dist[6] - dist[1], nvtxs);
  startvtx[2][5] =
      findindex(indices[2][5], vals[2][5], dist[5] - dist[2], nvtxs);
  startvtx[3][4] =
      findindex(indices[3][4], vals[3][4], dist[4] - dist[3], nvtxs);
  ...
  for (i = 1; i <= nvtxs; i++) {
    ...
    for (j = 0; j < nsets; j++) {
      val =
          -dist[j] +
          2 * (signx * xvecs[1][i] + signy * xvecs[2][i] + signz * xvecs[3][i]);
      if (j == 0 || val < bestval) {
        bestval = val;
        bestset = (short)j;
      }
