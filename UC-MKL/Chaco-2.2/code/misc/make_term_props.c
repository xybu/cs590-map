...
void make_term_props(
    graph, sub_nvtxs, loc2glob, assignment, architecture, ndims_tot, ndims,
    set_info, setnum, nsets, set_max, subsets, term_wgts,
    using_ewgts) struct vtx_data **graph; /* data structure for graph */
...
static void avg_dists_cube(
    ndims_tot, ndims, set_info, nsets, set_max, subsets,
    dists) int ndims_tot;  /* total number of hypercube dimensions */
...
static void avg_dists_mesh(architecture, set_info, nsets, set_max, subsets,
                           dists) int architecture; /* dimensions of mesh */
struct set_info *set_info; /* data about all the sets */
int nsets;                 /* number of subsets being created */
int set_max;               /* largest set created so far */
short *subsets;            /* subsets being created */
float *dists[MAXSETS];     /* distances from my subsets to other sets */
{
  ...
  for (i = 1; i < nsets; i++) {
    ...
    for (set = 0; set < set_max; set++) {
      if (set_info[set].span[0] >= 0) {
        val =
            avg_dist_mesh(&set_info[subsets[i]], &set_info[set], architecture);
...
static double avg_dist_mesh(
    set1, set2,
    architecture) struct set_info *set1; /* data about all first set */
...
int architecture;                        /* dimension of mesh */
{
  ...
  for (i = 0; i < architecture; i++) {
    val += avg_dist_interval(set1->low[i], set1->span[i], set2->low[i],
                             set2->span[i]);
  }
...
static double avg_dist_interval(
    set1_low, set1_span, set2_low,
    set2_span) short set1_low; /* lowest point for first interval */
