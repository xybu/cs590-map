...
/* NOTE: Uses a modified form of the bidirectional recurrence similar to Parlett
         ...
         Tinvit, Eispack's inverse iteration routine if the bidirectional
   recurrence
         fails to meet tolerance. Switches back to the bidirectional recurrence
   result
         if Tinvit fails to converge or gives a worse residual. There's code for
   the
...
/* NOTE: The residual calculations look too simple, but they are right because
         ...
         or backward recurrences or the merge point in the bidirectional
   recurrence
         where the resdual is not forced to zero. The values here are the
   residuals
...
/* NOTE: This routine is expected to return a normalized vector s such that it's
         last entry, s[j], is positive, hence the calls to sign_normalize().
   This
         is not an issue in the backward recurrence since we set s[j] positive.
   */

/* Finds eigenvector s of T and returns residual norm. */
double Tevec(alpha, beta, j, ritz,
             s) double *alpha; /* vector of Lanczos scalars */
...
double *s;                     /* approximate eigenvector of T */
{
  ...
  if (residual > SRESTOL) {
    ...
    tinvit_(&long_j, &long_j, &(alpha[1]), &(beta[1]), &(work[1]), &nevals,
            &(w[1]), &(index[1]), &(s[1]), &ierr, &(work[j + 1]),
            &(work[(2 * j) + 1]), &(work[(3 * j) + 1]), &(work[(4 * j) + 1]),
            &(work[(5 * j) + 1]));
