...
void eigensolve(graph, nvtxs, nedges, maxdeg, vwgt_max, vwsqrt, using_vwgts,
                using_ewgts, term_wgts, igeom, coords, yvecs, evals,
                architecture, assignment, goal, solver_flag, rqi_flag, vmax,
                ndims, mediantype,
                eigtol) struct vtx_data **graph; /* graph data structure */
...
double eigtol;      /* tolerance on eigenvectors */
{
  ...
  // REVISED //
  /*printf("This is my eigensolve.c Now goal is: \n");
  for (i = 0; i < (1<<ndims); i++) {
          printf("goal[%d]: %f\n", i, goal[i]);
  }*/
  ...
  if (term_wgts[1] != NULL && ndims == 1) { /* then use lanczos_ext */
    ...
    if (LANCZOS_SO_PRECISION == 2) { /* double precision */
      normal = lanczos_ext(graph, nvtxs, ndims, yvecs, eigtol, vwsqrt, maxdeg,
                           version, g, sigma);
    ...
    if (normal) {
      if (WARNING_EVECS > 2) {
        strout(
            "WARNING: Not an extended eigenproblem; switching to standard "
            "eigensolver.\n");
      }
    } else {
      if (w2 != w1) {
  ...
  if (normal) {
    if (rqi_flag) {
      ...
      step = 0;
      dummy_twgt[1] = NULL;
      coarsen(graph, nvtxs, nedges, using_vwgts, using_ewgts, dummy_twgt, igeom,
      ...
      if (solver_flag == 2) {
        ...
        if (LANCZOS_SO_PRECISION == 2) { /* double precision */
          ...
          if (yvecs != NULL) {
                  for (i = 1; i <= ndims; i++) {
                          for (j = 1; j <= nvtxs; j++) printf("%f\t",
          yvecs[i][j]);
                          printf("\n--------------------------------------------------\n");
                  }
          }*/
          /////////////////
          lanczos_SO(graph, nvtxs, ndims, yvecs, evals, bound, eigtol, vwsqrt,
                     maxdeg, version, architecture, nsets, assignment, active,
          ...
          lanczos_SO_float(graph, nvtxs, ndims, yvecs, evals, bound, eigtol,
                           vwsqrt, maxdeg, version, architecture, nsets,
