...
void divide(graph, nvtxs, nedges, using_vwgts, using_ewgts, vwsqrt, igeom,
            coords, assignment, goal, architecture, term_wgts, global_method,
            local_method, rqi_flag, vmax, ndims, eigtol, hop_mtx, nsets,
            striping) struct vtx_data **graph; /* graph data structure */
  ...
  if (global_method == 1) { /* Multilevel method. */
    ...
    if (VERTEX_SEPARATOR) {
      coarsen_klv(graph, nvtxs, nedges, using_vwgts, using_ewgts, term_wgts,
                  igeom, coords, vwgt_max, assignment, goal, architecture,
                  hop_mtx, solver_flag, ndims, nsets, vmax, mediantype,
                  mkconnected, eigtol, COARSE_NLEVEL_KL, 0, &bndy_list, weights,
                  FALSE);
      ...
      // REVISED //
      /*printf("This is my divide.c Now goal is: \n");
      for (i = 0; i < nsets; i++) {
              printf("goal[%d]: %f\n", i, goal[i]);
      }*/
    ...
    eigensolve(graph, nvtxs, nedges, maxdeg, vwgt_max, vwsqrt, using_vwgts,
               ...
               architecture, assignment, fake_goal, solver_flag, rqi_flag, vmax,
    ...
    inertial(graph, nvtxs, architecture, nsets, igeom, coords, assignment, goal,
             using_vwgts);
  }
