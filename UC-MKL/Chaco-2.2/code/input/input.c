...
char *inname_host; /* name of host input file ----------- ADD! */
...
int *ndims;         /* number of divisions at each stage */
{
  ...
  *fin = NULL;
  while (*fin == NULL) {
    if (PROMPT) printf("Graph input file: ");
    scanf("%s", inname);
  ...
  /*******ADD*******/
  /* Name and open input host file. */
  *fin_host = NULL;
  while (*fin_host == NULL) {
    if (PROMPT) printf("Host input file: ");
    scanf("%s", inname_host);

    *fin_host = fopen(inname_host, "r");
    if (*fin_host == NULL) {
  ...
  if (OUTPUT_ASSIGN && !SEQUENCE) {
    if (PROMPT) printf("Assignment output file: ");
    scanf("%s", outassignname);
  }

  /* Name output results file. */
  if (ECHO < 0) {
    if (PROMPT) printf("File name for saving run results: ");
    scanf("%s", outfilename);
  }
  ...
  *finassign = NULL;
  ...
  if (*global_method == 7) { /* Name and open input assignment file. */
    while (*finassign == NULL) {
      if (PROMPT) printf("Assignment input file: ");
    ...
    while (*fingeom == NULL) {
      if (PROMPT) printf("Geometry input file name: ");
      scanf("%s", geomname);
    ...
    if (eigensolver == 1) {
      ...
      *vmax = 0;
      while (*vmax <= 0) {
        if (PROMPT) printf("Number of vertices to coarsen down to: ");
        *vmax = input_int();
      }
    }
  ...
  *local_method = 0;
  ...
  if (*architecture == 0) {
    ...
    *ndims_tot = 0;
    while (*ndims_tot < 1) {
      if (PROMPT) printf("Total number of target hypercube dimensions: ");
      *ndims_tot = input_int();
      if (*ndims_tot < 1) {
    ...
    mesh_dims[1] = mesh_dims[2] = 1;
    if (*architecture == 2) {
      if (PROMPT) printf("X and Y extent of of 2-D mesh: ");
      mesh_dims[0] = input_int();
      mesh_dims[1] = input_int();
    } else if (*architecture == 3) {
      if (PROMPT) printf("X, Y and Z extent of 3-D mesh: ");
      mesh_dims[0] = input_int();
      mesh_dims[1] = input_int();
      mesh_dims[2] = input_int();
    } else { /* Anything else => 1-D mesh */
      if (PROMPT) printf("Size of 1-D mesh: ");
      mesh_dims[0] = input_int();
      *architecture = 1;
    }
  ...
  *ndims = 0;
  if (nprocs <= 3) {
    *ndims = 1;
    ...
    if (PROMPT) printf("Partitioning dimension: \n");
