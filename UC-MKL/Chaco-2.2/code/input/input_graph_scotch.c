...
int input_graph(fin, inname, start, adjacency, nvtxs, vweights,
                eweights) FILE *fin; /* input file */
...
float **eweights;                    /* edge weight list data */
{
  ...
  /* The (decimal) digits of the option variable mean: 1's digit not zero =>
     input
     edge weights 10's digit not zero => input vertex weights 100's digit not
     zero
  ...
  *eweights = NULL;
  ...
  while (!end_flag) j = read_int(fin, &end_flag);
  ...
  *start = (int *)smalloc((unsigned)(*nvtxs + 1) * sizeof(int));
  ...
  while ((narcs || using_vwgts || vtxnums) && end_flag != -1) {
    ...
    while (!end_flag) {
      ...
      if (!skip_flag) {
        found_flag = FALSE;
        for (j = (*start)[vertex - 1]; !found_flag && j < sum_edges + nedge;
             j++) {
          if ((*adjacency)[j] == neighbor) found_flag = TRUE;
        }
        if (found_flag) {
      ...
      if (using_ewgts) { /* Read edge weight if it's being input. */
        ...
        if (eweight <= 0 && CHECK_INPUT) {
          ...
          if (Output_File != NULL) {
            fprintf(Output_File,
                    "WARNING: Bad weight entered for edge (%d,%d).  Edge "
                    "ignored.\n",
                    vertex, neighbor);
          }
          skip_flag = TRUE;
      ...
      if (neighbor < vertex && !skip_flag) {
        found_flag = FALSE;
        for (j = (*start)[neighbor - 1]; !found_flag && j < (*start)[neighbor];
             j++) {
          if ((*adjacency)[j] == vertex) found_flag = TRUE;
        }
        if (!found_flag) {
          printf("ERROR in graph file `%s':", inname);
  ...
  if (vertex != 0) { /* Normal file was read. */
    ...
    if (nedges + 2 * self_edge != 2 * narcs &&
        ...
        nedges + self_edge + ignored != 2 * narcs && nedges != 2 * narcs &&
        nedges + ignored != 2 * narcs && CHECK_INPUT) {
      printf(
          "WARNING: I expected %d edges entered twice, but I only count %d.\n",
          narcs, nedges);
      if (Output_File != NULL) {
        fprintf(Output_File,
                "WARNING: I expected %d edges entered twice, but I only count "
                "%d.\n",
                narcs, nedges);
      }
    }
