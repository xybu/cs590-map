...
void opt3d(graph, yvecs, nvtxs, nmyvtxs, vwsqrt, ptheta, pphi, pgamma,
           using_vwgts) struct vtx_data *
    *graph;                     /* data structure containing vertex weights */
...
/* Compute rotation angle to minimize distance to discrete points. */
{
  ...
  for (ntries = 1; ntries <= maxtries; ntries++) {
    ...
    if (DEBUG_OPTIMIZE > 0) {
      printf(
          "On try %d, After %d (%d) passes, funcf=%e, funcc=%e (%f, %f, %f)\n",
