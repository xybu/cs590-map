...
void makecgraph2(
    graph, nvtxs, nedges, pcgraph, pcnvtxs, pcnedges, mflag, v2cv, nmerged,
    using_ewgts, igeom, coords,
    ccoords) struct vtx_data **graph; /* array of vtx data for graph */
  ...
  if (coords != NULL && ccoords != NULL && igeom > 0) {
    ...
    for (j = 1; j <= nvtxs; j++) {
      if (mflag[j] == 0) { /* If unmatched, leave it alone. */
        ...
        for (i = 0; i < igeom; i++) {
          ccoords[i][v2cv[j]] =
              (m1 * coords[i][v1] + m2 * coords[i][v2]) / (m1 + m2);
        }
      }
    }
  }
