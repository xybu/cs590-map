...
int reformat(start, adjacency, nvtxs, pnedges, vwgts, ewgts,
             pgraph) int *start; /* start of edge list for each vertex */
...
struct vtx_data ***pgraph;       /* ptr to array of vtx data for graph */
{
  ...
  /////////////
  // printf("start %d, ad %d, n %d, pn %d, vw %d, ew %d, pg %d\n", (int)start,
  // (int)adjacency, (int)nvtxs, (int)pnedges, (int)vwgts, (int)ewgts,
  // (int)pgraph);
  using_ewgts = (ewgts != NULL);
  using_vwgts = (vwgts != NULL);

  graph = (struct vtx_data **)smalloc_ret((unsigned)(nvtxs + 1) *
  ...
  *pgraph = graph;
  ...
  links = (struct vtx_data *)smalloc_ret((unsigned)(nvtxs) *
                                         sizeof(struct vtx_data));
  ...
  if (start != NULL) {
    ...
    // eptr = adjacency;
    // printf("%d!!!!!!!!! %d, %d\n", (int)eptr, (int)adjacency, (int)start[0]);
    wptr = ewgts;
  }
  self_edge = 0;
