...
void assign(
    graph, yvecs, nvtxs, ndims, cube_or_mesh, nsets, wsqrt, sets, active,
    mediantype, goal,
    vwgt_max) struct vtx_data **graph; /* data structure with vtx weights */
double **yvecs;   /* ptr to list of y-vectors (lengths nvtxs+1) Siyuan: This is
                     vectors used to do partitioning */
...
int vwgt_max;     /* largest vertex weight */
{
  ...
  // REVISED //
  /*int i = 0;
  printf("This is my assign.c Now goal is: \n");
  for (i = 0; i < (1<<ndims); i++) {
          printf("goal[%d]: %f\n", i, goal[i]);
  }*/
