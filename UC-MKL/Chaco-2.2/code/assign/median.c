...
void median(
    graph, vals, nvtxs, active, goal, using_vwgts,
    sets) struct vtx_data **graph; /* data structure with vertex weights */
...
short *sets;                       /* set each vertex gets assigned to */
{
  ...
  *aptr++ = 1;
  ...
  while (!done) {
    ...
    if (wlow + wbelow - lweight > whigh + wabove - uweight &&
      ...
      for (i = 0; i < myactive; i++) {
        if (vals[*aptr] >= guess) {
          ++removed;
          if (vals[*aptr] == guess) nearup = guess;
        } else
          *aptr2++ = *aptr;
        aptr++;
      }
    ...
    /*	else if ( whigh + wabove - uweight > wlow + wbelow - lweight) {*/
      ...
      for (i = 0; i < myactive; i++) {
        if (vals[*aptr] <= guess) {
          ++removed;
          if (vals[*aptr] == guess) neardown = guess;
        } else
          *aptr2++ = *aptr;
        aptr++;
      }
  ...
  median_assign(graph, vals, nvtxs, goal, using_vwgts, sets, wlow, whigh,
                guess);
}

void median_assign(
    graph, vals, nvtxs, goal, using_vwgts, sets, wlow, whigh,
    guess) struct vtx_data **graph; /* data structure with vertex weights */
double *vals;                       /* values of which to find median */
int nvtxs;                          /* number of values I own */
double *goal;                       /* desired sizes for sets */
int using_vwgts;                    /* are vertex weights being used? */
